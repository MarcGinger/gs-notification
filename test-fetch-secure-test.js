const http = require('http');

const options = {
  hostname: 'localhost',
  port: 3010,
  path: '/api/v1/notification/webhook-config/secure-tests/test-webhook-config-secretref-new',
  method: 'GET',
  headers: {
    Authorization:
      'Bearer eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICJDclR5eU44TkozdnVSMDdRbmZ3UVNCNWg5LUtEU2x1Q2cxNFBtMFU4WHE4In0..G8LVlWxo-Jy7rNBVgNGfVWzLHCjMqM3wMyLJUthbP5YHpeMnMbZHZmJyuLCzTwG5awxqhQfJ81ltooab9ffFL5GhP2tnLmpyCthUZZtrm7bQTQc8ZmFRfBjodWL6wErbD5ROvxM6iXxIiOqNODIGxyvE2YIzT-rPwaxYq3ESHGcEPVbij9d2pNtbWDuofEq55cbJDq9GZMVfnPPnkqLtwus3P-b19k9JLyPF8N9yR5NGbIPTv2qRvRP-Tu-_oTKr4c0eewvKzeuJBF_DZ2EMWccyS5M34eo_SPMx4dW9dC9xUE1dlUSRZKn6MpFbmjWOc0WBtz23AM_XXU4GEud2vQ',
  },
};

const req = http.request(options, (res) => {
  console.log(`Status: ${res.statusCode}`);
  console.log(`Headers: ${JSON.stringify(res.headers)}`);

  let data = '';
  res.on('data', (chunk) => {
    data += chunk;
  });

  res.on('end', () => {
    console.log('\n=== GET RESPONSE (SecretRef Decryption Test) ===');
    try {
      const jsonResponse = JSON.parse(data);
      console.log(JSON.stringify(jsonResponse, null, 2));

      // Check if sensitive fields are properly decrypted
      const hasSensitiveFields =
        jsonResponse.signingSecret ||
        jsonResponse.username ||
        jsonResponse.password;
      console.log('\n🔍 SecretRef Decryption Analysis:');
      console.log('- signingSecret present:', !!jsonResponse.signingSecret);
      console.log('- username present:', !!jsonResponse.username);
      console.log('- password present:', !!jsonResponse.password);
      console.log(
        '- Decryption working:',
        hasSensitiveFields ? '✅ YES' : '❌ NO',
      );
    } catch (error) {
      console.log('Raw response:', data);
    }
  });
});

req.on('error', (e) => {
  console.error(`Request error: ${e.message}`);
});

req.end();

console.log(
  '🔍 Testing SecretRef decryption by fetching the created SecureTest...',
);
console.log(
  '📥 GET /api/v1/notification/webhook-config/secure-tests/test-webhook-config-secretref-new',
);
