const http = require('http');

const options = {
  hostname: 'localhost',
  port: 3010,
  path: '/api/v1/notification/webhook-config/secure-tests/test-webhook-config-secretref',
  method: 'GET',
  headers: {
    Authorization:
      'Bearer eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICJDclR5eU44TkozdnVSMDdRbmZ3UVNCNWg5LUtEU2x1Q2cxNFBtMFU4WHE4In0..qg0OKSrf5l83XmRmcgvPEGs-nMao1FkwFQ3OceuBTbDqBNZCkThy8n9aTftVQfKG7o02GoArIPcv2osfcr0z3ww-1xBIxmSHE80lJtT5ftTNuLHtMSC3qHVemQvpqo6dIDFnG5TJfvLNXUKMSh7Jh76IT2Msjp8WISyXOwYO0y9tkOOnFduQuTawdJRxdRWH4oEQAavqT3PqZvkbFnX3x5TEKOcVEMUXRnqH2jkFrO1XYeQ5Q4n71cyzIO_HYuA5kmDIwNajojFQooP1ic1JTH22je-0VIrUWhFCrcbAgviYK_sCT-DrDWdd5tDpPGZAwVDq7qVfV5nErcKaMbATBQ',
  },
};

const req = http.request(options, (res) => {
  console.log(`Status: ${res.statusCode}`);
  console.log(`Headers: ${JSON.stringify(res.headers)}`);

  let data = '';
  res.on('data', (chunk) => {
    data += chunk;
  });

  res.on('end', () => {
    console.log('\n=== GET RESPONSE (SecretRef Decryption Test) ===');
    try {
      const jsonResponse = JSON.parse(data);
      console.log(JSON.stringify(jsonResponse, null, 2));

      // Check if sensitive fields are properly decrypted
      const hasSensitiveFields =
        jsonResponse.signingSecret ||
        jsonResponse.username ||
        jsonResponse.password;
      console.log('\n🔍 SecretRef Decryption Analysis:');
      console.log('- signingSecret present:', !!jsonResponse.signingSecret);
      console.log('- username present:', !!jsonResponse.username);
      console.log('- password present:', !!jsonResponse.password);
      console.log(
        '- Decryption working:',
        hasSensitiveFields ? '✅ YES' : '❌ NO',
      );
    } catch (error) {
      console.log('Raw response:', data);
    }
  });
});

req.on('error', (e) => {
  console.error(`Request error: ${e.message}`);
});

req.end();

console.log(
  '🔍 Testing SecretRef decryption by fetching the created SecureTest...',
);
console.log(
  '📥 GET /api/v1/notification/webhook-config/secure-tests/test-webhook-config-secretref',
);
