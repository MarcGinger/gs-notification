...
/**
 * HTTP Problem Details Error Definitions
 * Defines all errors that can occur during HTTP problem operations
 */
const HttpProblemErrorDefinitions = {
  INVALID_STATUS_CODE: {
    title: 'Invalid HTTP Status Code',
    detail: 'The provided HTTP status code is not valid',
...
/**
 * HTTP problem processing error catalog with namespaced error codes
 */
const HttpProblemErrors = Object.fromEntries(
  Object.entries(HttpProblemErrorDefinitions).map(([key, errorDef]) => {
    const code = `HTTP_PROBLEM.${key}` as const;
    return [key, { ...errorDef, code }];
  }),
) as {
  [K in keyof typeof HttpProblemErrorDefinitions]: DomainError<`HTTP_PROBLEM.${Extract<K, string>}`>;
};
...
/**
 * Utility to create a Problem Details response from an HTTP status code safely
 * @param status HTTP status code
 * @param title Error title
 * @param detail Optional error detail
 * @param instance Optional URI identifying this specific occurrence
 * @param options Configuration options for the conversion
 * @returns Result with ProblemDetails on success, DomainError on failure
 */
export function httpStatusToProblemSafe(
  status: HttpStatus,
  title: string,
  detail?: string,
  instance?: string,
  options: ProblemOptions = {},
): Result<ProblemDetails, DomainError> {
  const config = { ...getDefaultProblemOptions(), ...options };

  // Validate status code
  if (!isValidHttpStatus(status)) {
    return err(
      withContext(HttpProblemErrors.INVALID_STATUS_CODE, {
        operation: 'httpStatusToProblemSafe',
        input: { status, title, detail, instance },
