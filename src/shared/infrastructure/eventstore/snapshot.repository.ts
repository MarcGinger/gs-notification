...
export class SnapshotRepository<State> {
  ...
  constructor(
    ...
    this.logger = componentLogger(this.moduleLogger, 'SnapshotRepository');
  }
  ...
  async loadLatest(streamId: string): Promise<{
    ...
    try {
      ...
      Log.error(this.logger, 'snapshot.loadLatest.failed', {
        method: 'loadLatest',
        streamId,
        error: e.message,
        stack: e.stack,
  ...
  async save(
    ...
    try {
      ...
      const e = error as Error;
      Log.error(this.logger, 'snapshot.save.failed', {
        method: 'save',
        streamId,
        snapshot: {
          aggregate: snapshot.aggregate,
  ...
  async delete(streamId: string): Promise<void> {
    try {
      ...
      const e = error as Error;
      Log.error(this.logger, 'snapshot.delete.failed', {
        method: 'delete',
        streamId,
        error: e.message,
        stack: e.stack,
