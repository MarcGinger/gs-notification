...
import { systemClock } from '../time/convenience';
...
export class RedisOutboxRepository implements OutboxRepository {
  ...
  async add(records: OutboxRecord[]): Promise<void> {
    ...
    try {
      ...
      Log.error(this.logger, 'Failed to add outbox records', {
        component: 'RedisOutboxRepository',
        method: 'add',
  ...
  async nextBatch(limit: number): Promise<OutboxRecord[]> {
    try {
      ...
      for (let i = 0; i < ids.length; i++) {
        ...
        try {
          records.push({
            ...
            // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment -- Reason: Metadata is stored as an opaque envelope in Redis and parsed here; callers are responsible for validating fields. Ticket: TICKET-REQUIRED
      ...
      const e = error as Error;
      Log.error(this.logger, 'Failed to get next batch', {
        component: 'RedisOutboxRepository',
        method: 'nextBatch',
        limit,
        error: e.message,
  ...
  async markPublished(ids: string[]): Promise<void> {
    ...
    try {
      ...
      const e = error as Error;
      Log.error(this.logger, 'Failed to mark records as published', {
        component: 'RedisOutboxRepository',
        method: 'markPublished',
        ids,
        error: e.message,
  ...
  async markFailed(
    ...
    try {
      ...
      if (attempts >= maxAttempts) {
        ...
        Log.error(this.logger, 'Outbox record moved to DLQ', {
          component: 'RedisOutboxRepository',
          method: 'markFailed',
          recordId: id,
          attempts,
          error: `Max attempts exceeded: ${errorMessage}`,
          finalStatus: 'dead',
        });
        return;
      }
      ...
      const nextRetryAt = new Date(systemClock.nowMs() + backoff);
      ...
      const e = error as Error;
      Log.error(this.logger, 'Failed to mark record as failed', {
        component: 'RedisOutboxRepository',
        method: 'markFailed',
        recordId: id,
        errorMessage,
  ...
  async retryFailed(batchSize = 100): Promise<OutboxRecord[]> {
    try {
      ...
      const e = error as Error;
      Log.error(this.logger, 'Failed to process retry records', {
        component: 'RedisOutboxRepository',
        method: 'retryFailed',
        error: e.message,
        stack: e.stack,
  ...
  createRecord(
    ...
    return {
      ...
      // Cast metadata into the OutboxMetadata envelope. We accept unknown here because callers may pass opaque data; casting documents intent and aligns with Outbox persistence format.

      metadata: metadata as import('./outbox.entity').OutboxMetadata,
      status: 'pending',
      attempts: 0,
      createdAt: now,
      updatedAt: now,
  ...
  async getStats(): Promise<{
    ...
    try {
      ...
      const e = error as Error;
      Log.error(this.logger, 'Failed to get outbox stats', {
        component: 'RedisOutboxRepository',
        method: 'getStats',
        error: e.message,
        stack: e.stack,
  ...
  async cleanup(olderThanDays: number): Promise<number> {
    try {
      ...
      const e = error as Error;
      Log.error(this.logger, 'Failed to cleanup outbox records', {
        component: 'RedisOutboxRepository',
        method: 'cleanup',
        olderThanDays,
        error: e.message,
  ...
  async recoverProcessing(): Promise<number> {
    try {
      ...
      const e = error as Error;
      Log.error(this.logger, 'Failed to recover processing records', {
        component: 'RedisOutboxRepository',
        method: 'recoverProcessing',
        error: e.message,
        stack: e.stack,
