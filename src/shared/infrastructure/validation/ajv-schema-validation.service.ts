...
const COMPONENT = 'AjvSchemaValidationService';

/**
 * AJV Schema Validation Service
 *
 * Uses intentional throw pattern for private helper methods:
 * - Private methods (getOrCompile) throw on failure for simplicity
 * - Public methods catch throws and convert to structured SchemaValidationResult
 * - This is standard pattern for validation libraries where schema compilation failure is exceptional
 */
@Injectable()
export class AjvSchemaValidationService {
  ...
  constructor(@Inject(APP_LOGGER) private readonly moduleLogger: Logger) {
    // component-scoped child logger so payloads don't need service/component fields
  ...
  validateSchema(schema: JsonSchema): SchemaValidationResult {
    try {
      ...
      Log.warn(this.logger, 'ajv.schema.invalid', {
  ...
  validateData(
    ...
    try {
      ...
      if (!ok) {
        ...
        Log.debug(this.logger, 'ajv.data.invalid', {
          method: 'validateData',
      ...
      Log.error(this.logger, 'ajv.data.error', {
        method: 'validateData',
        schemaId: (schema as JsonSchema).$id,
  ...
  private getOrCompile(schema: JsonSchema): ValidateFunction {
    ...
    if (schema.$id) {
      ...
      // INTENTIONAL THROW: AJV schema loading failure - caught by public methods and converted to SchemaValidationResult
  ...
  private validateTemplateProperties(
    ...
    for (const [key, prop] of Object.entries(properties)) {
      ...
      if (!prop.description) {
        Log.debug(this.logger, 'ajv.template.desc.recommendation', {
