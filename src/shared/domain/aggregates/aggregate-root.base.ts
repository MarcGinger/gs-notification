...
export abstract class AggregateRootBase {
   ...
   */
  protected validateInvariants(): Result<void, DomainError> {
   ...
   */
  protected executeBusinessLogic(
    operation: () => void,
    ...
    try {
      ...
      return {
        ok: false,
        error: {
          code: 'AGGREGATE.BUSINESS_LOGIC_ERROR',
...
export interface AggregateRepository<T extends AggregateRootBase> {
  load(id: string): Promise<Result<T, DomainError>>;
  save(
    aggregate: T,
