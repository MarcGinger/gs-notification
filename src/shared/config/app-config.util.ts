...
export class AppConfigUtil {
  ...
  static getPublicBaseUrl(): URL | null {
    // Support legacy PUBLIC_API_URL as well as APP_SERVER_PUBLIC_URL
    const raw = (
      process.env.PUBLIC_API_URL ??
      process.env.APP_SERVER_PUBLIC_URL ??
      ''
    ).trim();
    if (!raw) return null;
    try {
  ...
  static isContainerized(): boolean {
    // Accept a variety of environment variables used across platforms and tests
    if (process.env.INFRA_KUBERNETES_SERVICE_HOST?.trim()) return true;
    if (process.env.KUBERNETES_SERVICE_HOST?.trim()) return true;
    if (process.env.INFRA_CONTAINER_HOST?.trim()) return true;
    if (process.env.CONTAINER_HOST?.trim()) return true;
    if (process.env.INFRA_CONTAINER_DOCKER_ENABLED === 'true') return true;
    if (process.env.DOCKER_CONTAINER === 'true') return true;

    // Fallback: check for docker file marker if available (tests may mock fs.existsSync)
    try {
      // require inside function so tests can mock/replace existsSync
      // eslint-disable-next-line @typescript-eslint/no-require-imports -- Reason: runtime optional require to allow test mocking of fs.existsSync. Ticket: TICKET-REQUIRED
  ...
  static getSystemConfig() {
    return {
      ...
      bootTime: Number(process.env.BOOT_TIME ?? Date.now()),
    };
  }
  ...
  static getDatabaseConfig() {
    ...
    return {
      // Connection settings
      url: process.env.DATABASE_POSTGRES_URL, // Legacy fallback
      ...
      schema: process.env.DATABASE_POSTGRES_SCHEMA || 'public',
  ...
  static getLoggingConfig() {
    const additionalRedactKeys = process.env.LOGGING_CORE_REDACT_KEYS;
    return {
      ...
      environment: this.getEnvironment(),
      redactKeys: additionalRedactKeys || undefined,
      // Loki configuration
      loki: {
    ...
    return {
      pii: {
        ...
        pseudonymizationSalt:
          process.env.PII_PSEUDO_SALT || 'change-me-in-production',
        enableTokenization:
