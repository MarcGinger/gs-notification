...
export class ConfigValidator {
  ...
  /**
   * Get validated configuration safely with Result pattern
   */
  async getValidatedConfigSafe(
    options: ValidationOptions = {},
  ): Promise<Result<AppConfig, DomainError>> {
    if (this.validatedConfig && !options.source && !options.environment) {
      return ok(this.validatedConfig);
    }
    ...
    // validatedConfig is set when validation.valid === true above; assert non-null for the Result type
    return ok(this.validatedConfig as AppConfig);
  }
  ...
  static async migrateFromLegacyConfig(): Promise<{
    ...
    // Get new configuration safely
    const validator = ConfigValidator.getInstance();
    const migratedConfigResult = await validator.getValidatedConfigSafe();
    if (!migratedConfigResult.ok) {
      // INTENTIONAL THROW: Migration utility must fail fast on configuration errors
      throw new Error(
        `Configuration migration failed: ${migratedConfigResult.error.detail}`,
      );
    }
    const migratedConfig = migratedConfigResult.value;
 ...
 */
export async function getValidatedConfigSafe(
  options: ValidationOptions = {},
): Promise<Result<AppConfig, DomainError>> {
  const validator = ConfigValidator.getInstance();
  return validator.getValidatedConfigSafe(options);
}
