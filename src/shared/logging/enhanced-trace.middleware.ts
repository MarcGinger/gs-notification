...
import { APP_LOGGER, componentLogger, Logger } from './index';
...
export class EnhancedTraceMiddleware {
  ...
  constructor(
    ...
    this.logger = componentLogger(moduleLogger, 'EnhancedTraceMiddleware');
  }
  ...
  use(req: Request, res: Response, next: NextFunction) {
    ...
    this.cls.run(() => {
      ...
      // Log trace ID creation/reception
      this.logger.info(
        {
          service: 'middleware',
          component: 'EnhancedTraceMiddleware',
          method: 'use',
          operation: 'trace_id_set',
          trace: { id: traceId },
          source: req.headers['traceparent']
            ? 'traceparent_header'
