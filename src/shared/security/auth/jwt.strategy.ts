...
export class JwtStrategy extends PassportStrategy(Strategy) {
  constructor(
    ...
    // Get JWT configuration safely
    const jwtConfigResult = securityConfigService.getValidatedJwtConfigSafe();
    if (!jwtConfigResult.ok) {
      // INTENTIONAL THROW: Constructor must fail fast on configuration errors - can't proceed without valid config
      throw new Error(
        `JWT configuration error: ${jwtConfigResult.error.detail}`,
      );
    }
    const jwtConfig = jwtConfigResult.value;

    // Get JWKS URI safely
    const jwksUriResult = securityConfigService.getJwksUriSafe();
    if (!jwksUriResult.ok) {
      // INTENTIONAL THROW: Constructor must fail fast on configuration errors - can't proceed without valid JWKS URI
      throw new Error(
        `JWKS URI configuration error: ${jwksUriResult.error.detail}`,
      );
    }
    const jwksUri = jwksUriResult.value;
