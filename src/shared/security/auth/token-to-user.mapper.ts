...
export class TokenToUserMapper {
  ...
  mapToUserToken(payload: JwtPayload): IUserToken {
    ...
    const { tenant } = this.extractTenant(payload, this.options);
    ...
    const userToken: IUserToken = {
      ...
      // defensive: prefer explicit tenant_id, fallback to tenant claim when available
      tenant_id: payload.tenant_id ?? payload.tenant,
