...
export class SecurityMonitoringService {
   ...
   */
  resetMetrics(): void {
    // Reset each known metric property explicitly to avoid unsafe any access
    this.metrics.authorizationRequests = 0;
    this.metrics.authorizationDenials = 0;
    this.metrics.authenticationFailures = 0;
    this.metrics.emergencyAccess = 0;
    this.metrics.piiFieldsDetected = 0;
    this.metrics.suspiciousActivity = 0;
    this.metrics.policyViolations = 0;
  ...
  private triggerAlert(
    ...
    // Safely extract message field if present
    const rawMessage = alertData['message'];
    ...
    if (typeof rawMessage === 'string') {
      ...
      try {
        alertMessage = JSON.stringify(rawMessage);
      } catch {
        alertMessage = '[object]';
      }
    } else {
      // primitive fallback (number/boolean/symbol)
      alertMessage = String(rawMessage as number | boolean | symbol);
    }
    Log.warn(this.logger, `Security Alert: ${alertMessage}`, {
      method: 'triggerAlert',
      alertType,
      alertData,
      timestamp: new Date().toISOString(),
