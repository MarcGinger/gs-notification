 ...
 * Handles channel reference data lookups for FK validation in the Execute context using Redis projections
 ...
 *
 * @domain Notification Context - Execute Channel Reader Repository (Redis)
...
export class ChannelReaderRepository implements IChannelReader {
  ...
  private generateChannelKey(tenantId: string, id: string): string {
    ...
    return `notification:channel-projector:{${tenantId}}:channel:${id}`;
  }
  ...
  /**
   * Find valid Channel Ids for Channel configuration
   * @param actor - The authenticated user context
   * @param options - Optional repository options (correlation ID, timeout, etc.)
   * @returns Result containing available Channel Ids
   */
  async findValidChannelIds(
    actor: ActorContext,
    options?: RepositoryOptions,
  ): Promise<Result<string[], DomainError>> {
    const operation = 'findValidChannelIds';
    const riskLevel = this.assessOperationRisk(operation);
    const correlationId =
      options?.correlationId ??
      CorrelationUtil.generateForOperation('channel-list-ids');
    ...
    try {
      Log.debug(this.logger, 'Finding valid channel ids in Redis', {
        ...logContext,
        queryDetails: {
          scope: 'redis_keys',
          method: 'redis.scan',
          pattern: `channel-projector:{${actor.tenantId}}:channel:*`,
          clusterSafe: true,
        },
      });

      // Use Redis SCAN to find all channel keys for the tenant
      const pattern = `channel-projector:{${actor.tenantId}}:channel:*`;
      const ids: string[] = [];
      ...
      for await (const keys of scanIterator) {
        for (const key of keys as string[]) {
          // Extract id from key pattern: channel-projector:{tenantId}:channel:{ id }
  ...
  /**
   * Find Channel configuration by Channel id
   * @param actor - The authenticated user context
   * @param id - The Channel id to lookup
  ...
  async findChannelById(
    ...
    const operation = 'findChannelById';
    const riskLevel = this.assessOperationRisk(operation);
    const correlationId =
      options?.correlationId ??
    ...
    try {
      ...
      if (!id) {
        Log.warn(this.logger, 'Invalid channel id provided', {
          ...logContext,
          validationError: 'INVALID_CHANNEL_ID',
      ...
      const redisKey = this.generateChannelKey(actor.tenantId, id);
