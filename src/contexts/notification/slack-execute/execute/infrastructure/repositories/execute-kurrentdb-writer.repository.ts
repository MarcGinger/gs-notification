...
import { ExecuteDeletedEvent } from '../../domain/events';
import { IExecuteWriter } from '../../application/ports';

/**
 * Execute Writer Repository - Interface Segregation Principle Implementation
 *
 * Handles write operations (create, update, delete) for Execute aggregates.
 ...
 *
 * @domain Notification Context - Execute Writer Repository
 * @layer Infrastructure
 * @pattern Repository Pattern + Interface Segregation Principle
 */
export class ExecuteWriterRepository
  extends BaseWriterRepository
  implements IExecuteWriter
{
  ...
  constructor(
    ...
    super();
    this.loggingConfig = {
      serviceName: 'NotificationSlackExecuteService',
      component: 'ExecuteWriterRepository',
    };
    this.logger = componentLogger(baseLogger, this.loggingConfig.component);
  }

  /**
   * Save a Execute aggregate (create or update) using EventStoreDB-first approach
   * @param actor - The authenticated user context
   * @param execute - The Execute aggregate to save
  ...
  async save(
    ...
    const correlationId = CorrelationUtil.generateForOperation('execute-save');
    ...
    const logContext = RepositoryLoggingUtil.createLogContext(
      ...
      actor,
      {
        aggregateId: execute.id.value,
        currentVersion: execute.version,
        eventCount: execute.uncommittedEvents?.length ?? 0,
      },
    );
    ...
    const stream = this.buildStreamName(
      actor.tenantId,
      ExecuteProjectionKeys.getEventStoreStreamPrefix(),
      execute.id.value,
    );
    const events = execute.uncommittedEvents ?? [];
    ...
    if (events.length === 0) {
      ...
      const receipt: SaveReceipt = {
        ...
        eventCount: 0,
        newVersion: execute.version,
        streamRevision:
          execute.version === 0 ? BigInt(-1) : BigInt(execute.version - 1),
    ...
    const prevVersion = execute.version - eventsToStore.length;
  ...
  /**
   * Delete a Execute by its unique identifier using EventStoreDB-first approach
   * @param actor - The authenticated user context
   * @param id - The unique identifier of the Execute to delete
  ...
  async delete(
    ...
    const correlationId =
      ...
      CorrelationUtil.generateForOperation('execute-delete');
    ...
    const eventMetadata: EventMetadata = {
      ...
      service: 'execute-writer-repository',
      timestampIso: this.clock.nowIso(),
      eventVersion: '1.0',
    ...
    // Create proper ExecuteDeletedEvent
    const deletedEvent = ExecuteDeletedEvent.create(
      {
        id: id.value,
    ...
    try {
      Log.debug(
        this.logger,
        'Appending execute deletion event to EventStoreDB',
        {
          ...logContext,
          stream,
          executeId: id.value,
