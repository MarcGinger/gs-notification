...
import { IRequestReader } from '../../application/ports';

/**
 * Request Reader Repository - Redis Implementation
 *
 * Bounded Context: Notification/Request
 * Handles basic read operations for Request aggregates using Redis projections
 ...
 *
 * @domain Notification Context - Request Reader Repository (Redis)
 * @layer Infrastructure
 * @pattern Repository Pattern + Redis Projector
 */
@Injectable()
export class RequestReaderRepository implements IRequestReader {
  ...
  constructor(
    ...
    this.loggingConfig = {
      ...
      component: 'RequestReaderRepository',
    };
    this.logger = componentLogger(baseLogger, this.loggingConfig.component);

    Log.info(
      this.logger,
      'RequestReaderRepository initialized with Redis backend',
      {
  ...
  /**
   * Parse Redis hash data into RequestSnapshotProps
   */
  private parseRedisHashToRequest(
    hashData: Record<string, string>,
  ): RequestSnapshotProps | null {
    try {
      ...
      const status = hashData.status as RequestSnapshotProps['status'];
   ...
   * @returns Result containing the Request snapshot or null if not found
   */
  async findById(
    actor: ActorContext,
    code: RequestCode,
    options?: RepositoryOptions,
  ): Promise<Result<Option<RequestSnapshotProps>, DomainError>> {
    const operation = 'findById';
    const riskLevel = this.assessOperationRisk(operation);
    const correlationId =
      options?.correlationId ??
      CorrelationUtil.generateForOperation('request-find-by-id');
    ...
    RepositoryLoggingUtil.logAuthorizationSuccess(
      ...
      operation,
      logContext,
      { queryType: 'request_lookup', scope: 'redis_projection' },
    );

    try {
      // Generate cluster-safe Redis key
      const redisKey = this.generateRequestKey(actor.tenant, code.value);

      Log.debug(this.logger, 'Finding request by ID in Redis', {
        ...logContext,
        queryDetails: {
          scope: 'redis_hash',
      ...
      // Parse Redis hash to RequestSnapshot
      const requestSnapshot = this.parseRedisHashToRequest(hashData);
      ...
      return ok(Option.some(requestSnapshot));
    } catch (error) {
  ...
  /**
   * Check if a request exists by ID (for write-path validation)
   * @param actor - The authenticated user context
   * @param code - The unique identifier of the Request
   * @param options - Optional repository options
  ...
  async exists(
    ...
    const correlationId =
      options?.correlationId ??
      CorrelationUtil.generateForOperation('request-exists-check');
    ...
    try {
      ...
      Log.debug(this.logger, 'Checking request existence in Redis', {
        ...logContext,
        queryDetails: {
          scope: 'redis_hash',
          method: 'redis.exists',
  ...
  /**
   * Get request version for optimistic concurrency control
   * @param actor - The authenticated user context
   * @param code - The unique identifier of the Request
   * @param options - Optional repository options
  ...
  async getVersion(
    ...
    const correlationId =
      options?.correlationId ??
      CorrelationUtil.generateForOperation('request-get-version');
    ...
    try {
      ...
      Log.debug(this.logger, 'Getting request version from Redis', {
        ...logContext,
        queryDetails: {
          scope: 'redis_hash',
          method: 'redis.hmget',
  ...
  /**
   * Get minimal request data for write-path operations
   * @param actor - The authenticated user context
   * @param code - The unique identifier of the Request
   * @param options - Optional repository options
   * @returns Result containing minimal request data or null if not found
   */
  async getMinimal(
    actor: ActorContext,
    code: RequestCode,
    ...
    const correlationId =
      options?.correlationId ??
      CorrelationUtil.generateForOperation('request-get-minimal');
    ...
    try {
      ...
      Log.debug(this.logger, 'Getting minimal request data from Redis', {
        ...logContext,
        queryDetails: {
          scope: 'redis_hash',
          method: 'redis.hmget',
