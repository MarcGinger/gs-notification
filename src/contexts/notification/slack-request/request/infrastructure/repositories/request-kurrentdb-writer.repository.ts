...
import { RequestDeletedEvent } from '../../domain/events';
import { IRequestWriter } from '../../application/ports';

/**
 * Request Writer Repository - Interface Segregation Principle Implementation
 *
 ...
 *
 * @domain Notification Context - Request Writer Repository
 * @layer Infrastructure
 * @pattern Repository Pattern + Interface Segregation Principle
 */
export class RequestWriterRepository
  extends BaseWriterRepository
  implements IRequestWriter
{
  ...
  constructor(
    ...
    this.loggingConfig = {
      ...
      component: 'RequestWriterRepository',
    };
    this.logger = componentLogger(baseLogger, this.loggingConfig.component);
  }

  /**
   * Save a Request aggregate (create or update) using EventStoreDB-first approach
  ...
  async save(
    ...
    const correlationId = CorrelationUtil.generateForOperation('request-save');
    ...
    const logContext = RepositoryLoggingUtil.createLogContext(
      ...
      actor,
      {
        ...
        eventCount: request.uncommittedEvents?.length ?? 0,
      },
    );
    ...
    const stream = this.buildStreamName(
      actor.tenant,
      RequestProjectionKeys.getEventStoreStreamPrefix(),
      request.id.value,
    );
    const events = request.uncommittedEvents ?? [];
    ...
    if (events.length === 0) {
      ...
      const receipt: SaveReceipt = {
        ...
        streamRevision:
          request.version === 0 ? BigInt(-1) : BigInt(request.version - 1),
    ...
    const prevVersion = request.version - eventsToStore.length;
  ...
  /**
   * Delete a Request by its unique identifier using EventStoreDB-first approach
   * @param actor - The authenticated user context
  ...
  async delete(
    ...
    const correlationId =
      ...
      CorrelationUtil.generateForOperation('request-delete');
    ...
    const eventMetadata: EventMetadata = {
      ...
      service: 'request-writer-repository',
      timestampIso: this.clock.nowIso(),
      eventVersion: '1.0',
    ...
    // Create proper RequestDeletedEvent
    const deletedEvent = RequestDeletedEvent.create(
      {
        code: code.value,
    ...
    try {
      Log.debug(
        this.logger,
        'Appending request deletion event to EventStoreDB',
        {
          ...logContext,
          stream,
          requestCode: code.value,
