...
/**
 * Request documentation endpoints interface
 */
export interface RequestDocumentationUrls {
  request: string;
}

/**
 * Request Documentation
 * This module handles the Swagger documentation for requests
 *
 */
export class RequestDocumentation {
  static setup(app: INestApplication, port: string | number): void {
    const config = new DocumentBuilder()
      ...
      .setTitle('📱 Request Management API')
      .setDescription(
        `
...
`,
      ...
      .setVersion('1.0.0')
      .addTag('Requests', `Configuration for request table`);
    ...
    SwaggerModule.setup(
      'api/docs/notification/slack-request/request',
      app,
      document,
    );
  }
  ...
  /**
   * Setup all request-related documentation endpoints
   * This method centralizes the setup of all request module documentation
   */
  static setupAll(
    app: INestApplication,
    port: string | number,
  ): RequestDocumentationUrls {
    // Setup main request documentation
    RequestDocumentation.setup(app, port);

    // Setup sub-module documentation

    // Return all endpoint URLs
    return {
      request: RequestDocumentation.getEndpoint(port),
    };
  }

  static getEndpoint(port: string | number): string {
    return `${SwaggerConfigUtil.getServerUrl(port)}/api/docs/notification/slack-request/request`;
  }
}
