...
import { IGetRequestUseCase } from './contracts';
import { RequestErrors } from '../../domain/errors';

/**
 * Get Request Use Case - CQRS Read Side Implementation
 *
 ...
 * The query port returns read-optimized DetailRequestResponse DTOs directly.
 */
...
/**
 * ✅ Enhanced Get Request Use Case with Modern Security and Compliance
 *
 * Features:
...
export class GetRequestUseCase implements IGetRequestUseCase {
  ...
  async execute(params: {
    ...
    const authResult = await this.authorizationService.canReadResource(
      params.user.sub,
      params.code, // Using code as request identifier
      correlationId,
      {
    ...
    if (
      ...
      const validationError = {
        code: 'REQUEST.INVALID_THE_CODE' as const,
    ...
    const repositoryOptions: RepositoryOptions = {
      cache: {
        ttl: CACHE_TTL.STANDARD, // 5 minutes cache for request reads
    ...
    const requestResult = await this.query.findById(
      actor,
      params.code,
      repositoryOptions,
    );
    ...
    if (!isOk(requestResult)) {
      const contextError = withContext(requestResult.error, {
        correlationId,
        userId: params.user.sub,
        operation: 'get_request',
        code: params.code,
    ...
    // Step 5: Handle Option<DetailRequestResponse> safely - query port already returns DTOs (proper CQRS)
    const requestDto = Option.isNone(requestResult.value)
      ? null
      : requestResult.value.value;

    // Step 5.1: Return 404 error if request not found
    if (!requestDto) {
      const notFoundError = withContext(RequestErrors.REQUEST_NOT_FOUND, {
        code: params.code,
