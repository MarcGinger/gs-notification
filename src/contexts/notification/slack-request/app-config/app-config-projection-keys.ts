...
export class AppConfigProjectionKeys {
   ...
   * Example: slack-request.app-config.v1
   */
   ...
   * Example: $ce-slack-request.app-config.v1
   */
   ...
   * Example: slack-request.app-config.v1-tenant-12345
   */
   ...
   * Format: notification.slack:v1:{tenant}:app-config:{appConfigId}
   */
  static getRedisAppConfigKey(tenant: string, appConfigId: string): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:{${tenant}}:app-config:${appConfigId}`;
  }

  /**
   * Get Redis key for app-config index with cluster-safe hash tags
   * Format: notification.slack:v1:{tenant}:idx:app-config:list
   */
  static getRedisAppConfigIndexKey(tenant: string): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:{${tenant}}:idx:app-config:list`;
  }

  /**
   * Get Redis key for category-based app-config index with cluster-safe hash tags
   * Format: notification.slack:v1:{tenant}:idx:app-config:by-category:{category}
   */
  static getRedisCategoryIndexKey(tenant: string, category: string): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:{${tenant}}:idx:app-config:by-category:${category}`;
  }

  /**
   * Get Redis key pattern for all app-configs in a tenant
   * Format: notification.slack:v1:{tenant}:app-config:*
   */
  static getRedisTenantAppConfigPattern(tenant: string): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:{${tenant}}:app-config:*`;
  }
   ...
   * Format: notification.slack:pp:ver:{tenant}:app-config:{appConfigId}
   */
  static getNamespacedVersionHintKey(
    tenant: string,
    appConfigId: string,
  ): string {
    return `notification.slack:pp:ver:{${tenant}}:app-config:${appConfigId}`;
  }
