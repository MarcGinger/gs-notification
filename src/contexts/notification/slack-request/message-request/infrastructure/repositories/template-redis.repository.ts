 ...
 * Handles template reference data lookups for FK validation in the MessageRequest context using Redis projections
 ...
 *
 * @domain Notification Context - MessageRequest Template Reader Repository (Redis)
...
export class TemplateReaderRepository implements ITemplateReader {
   ...
   */
  private generateTemplateKey(tenant: string, code: string): string {
    // ✅ Hash-tags ensure key routes to same Redis Cluster slot as projector
    return `notification.slack:v1:{${tenant}}:template:${code}`;
  }
    ...
    try {
      Log.debug(this.logger, 'Finding valid template codes in Redis', {
        ...
        queryDetails: {
          scope: 'redis_keys',
          method: 'redis.scan',
          pattern: `template-projector:{${actor.tenant}}:template:*`,
          clusterSafe: true,
        },
      });

      // Use Redis SCAN to find all template keys for the tenant
      const pattern = `template-projector:{${actor.tenant}}:template:*`;
      const codes: string[] = [];
      ...
      for await (const keys of scanIterator) {
        for (const key of keys as string[]) {
          // Extract code from key pattern: template-projector:{tenant}:template:{ code }
  ...
  async findTemplateByCode(
    ...
    try {
      ...
      const redisKey = this.generateTemplateKey(actor.tenant, code);
