...
export class MessageRequestReaderRepository implements IMessageRequestReader {
   ...
   */
  private generateMessageRequestKey(tenantId: string, id: string): string {
    // ✅ Use centralized key generation for consistency
    return MessageRequestProjectionKeys.getRedisMessageRequestKey(tenantId, id);
  }
  ...
  async findById(
    ...
    try {
      ...
      const redisKey = this.generateMessageRequestKey(actor.tenantId, id.value);
  ...
  async exists(
    ...
    try {
      // Generate cluster-safe Redis key
      const redisKey = this.generateMessageRequestKey(
        actor.tenantId!,
        id.value,
      );
