...
import { MessageRequestDeletedEvent } from '../../domain/events';
import { IMessageRequestWriter } from '../../application/ports';

/**
 * MessageRequest Writer Repository - Interface Segregation Principle Implementation
 *
 ...
 *
 * @domain Notification Context - MessageRequest Writer Repository
 * @layer Infrastructure
 * @pattern Repository Pattern + Interface Segregation Principle
 */
export class MessageRequestWriterRepository
  extends BaseWriterRepository
  implements IMessageRequestWriter
{
  ...
  constructor(
    ...
    this.loggingConfig = {
      ...
      component: 'MessageRequestWriterRepository',
    };
    this.logger = componentLogger(baseLogger, this.loggingConfig.component);
  }

  /**
   * Save a MessageRequest aggregate (create or update) using EventStoreDB-first approach
  ...
  async save(
    ...
    const logContext = RepositoryLoggingUtil.createLogContext(
      ...
      actor,
      {
        aggregateId: messageRequest.id.value,
        currentVersion: messageRequest.version,
        eventCount: messageRequest.uncommittedEvents?.length ?? 0,
      },
    );
    ...
    const stream = this.buildStreamName(
      actor.tenant,
      MessageRequestProjectionKeys.getEventStoreStreamPrefix(),
      messageRequest.id.value,
    );
    const events = messageRequest.uncommittedEvents ?? [];
    ...
    if (events.length === 0) {
      ...
      const receipt: SaveReceipt = {
        ...
        eventCount: 0,
        newVersion: messageRequest.version,
        streamRevision:
          messageRequest.version === 0
            ? BigInt(-1)
            : BigInt(messageRequest.version - 1),
    ...
    const prevVersion = messageRequest.version - eventsToStore.length;
    ...
    const envelopes = this.toEnvelopes(
      ...
      eventsToStore,
      actor,
      'message-request-writer-repository',
    ...
    try {
      ...
      messageRequest.markEventsAsCommitted();
  ...
  /**
   * Delete a MessageRequest by its unique identifier using EventStoreDB-first approach
   * @param actor - The authenticated user context
  ...
  async delete(
    ...
    const correlationId =
      ...
      CorrelationUtil.generateForOperation('message-request-delete');
    ...
    RepositoryLoggingUtil.logAuthorizationSuccess(
      ...
      logContext,
      {
        ...
        scope: 'message-request_deletion',
        messageRequestId: id.value,
      },
    );
    ...
    const eventMetadata: EventMetadata = {
      ...
      service: 'message-request-writer-repository',
      timestampIso: this.clock.nowIso(),
      eventVersion: '1.0',
    ...
    // Create proper MessageRequestDeletedEvent
    const deletedEvent = MessageRequestDeletedEvent.create(
      {
        id: id.value,
    ...
    try {
      Log.debug(
        this.logger,
        'Appending message-request deletion event to EventStoreDB',
        {
          ...logContext,
          stream,
          messageRequestId: id.value,
      ...
      RepositoryLoggingUtil.logQueryMetrics(
        ...
        logContext,
        {
          ...
          sampleData: {
            ...
            messageRequestId: id.value,
            streamRevision: appendResult?.nextExpectedRevision?.toString(),
            eventType: 'MessageRequestDeletedEvent',
          },
        },
      );
