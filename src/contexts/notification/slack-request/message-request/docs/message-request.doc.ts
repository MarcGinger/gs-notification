...
/**
 * MessageRequest documentation endpoints interface
 */
export interface MessageRequestDocumentationUrls {
  messageRequest: string;
}

/**
 * MessageRequest Documentation
 * This module handles the Swagger documentation for message_requests
 *
 */
export class MessageRequestDocumentation {
  static setup(app: INestApplication, port: string | number): void {
    const config = new DocumentBuilder()
      ...
      .setTitle('📱 MessageRequest Management API')
      .setDescription(
        `
...
`,
      ...
      .setVersion('1.0.0')
      .addTag('MessageRequests', `Configuration for message_request table`);

    // Add dynamic server configuration
    SwaggerConfigUtil.addServers(config, port);

    const document = SwaggerModule.createDocument(app, config.build(), {
      include: [MessageRequestModule],
      extraModels: [],
    });

    SwaggerModule.setup(
      'api/docs/notification/slack-request/message-request',
      app,
      document,
    );
  }
  ...
  /**
   * Setup all messageRequest-related documentation endpoints
   * This method centralizes the setup of all messageRequest module documentation
   */
  static setupAll(
    app: INestApplication,
    port: string | number,
  ): MessageRequestDocumentationUrls {
    // Setup main messageRequest documentation
    MessageRequestDocumentation.setup(app, port);

    // Setup sub-module documentation

    // Return all endpoint URLs
    return {
      messageRequest: MessageRequestDocumentation.getEndpoint(port),
    };
  }

  static getEndpoint(port: string | number): string {
    return `${SwaggerConfigUtil.getServerUrl(port)}/api/docs/notification/slack-request/message-request`;
  }
}
