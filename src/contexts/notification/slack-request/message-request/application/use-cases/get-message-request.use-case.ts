...
import { IGetMessageRequestUseCase } from './contracts';
import { MessageRequestErrors } from '../../domain/errors';

/**
 * Get MessageRequest Use Case - CQRS Read Side Implementation
 *
 ...
 * The query port returns read-optimized DetailMessageRequestResponse DTOs directly.
 */
...
/**
 * ✅ Enhanced Get MessageRequest Use Case with Modern Security and Compliance
 *
 * Features:
...
@Injectable()
export class GetMessageRequestUseCase implements IGetMessageRequestUseCase {
  ...
  constructor(
    ...
    this.loggingConfig = {
      ...
      component: 'GetMessageRequestUseCase',
  ...
  async execute(params: {
    ...
    const operation = 'get_message_request';
    ...
    const authResult = await this.authorizationService.canReadResource(
      params.user.sub,
      params.id, // Using id as messageRequest identifier
      correlationId,
      {
    ...
    if (
      ...
      const validationError = {
        code: 'MESSAGE_REQUEST.INVALID_THE_CODE' as const,
        title: 'Invalid messageRequest id',
        category: 'validation' as const,
        context: {
          id: params.id,
          correlationId,
          userId: params.user.sub,
          operation: 'get_messageRequest',
        },
      };
      UseCaseLoggingUtil.logOperationError(
        this.logger,
    ...
    const repositoryOptions: RepositoryOptions = {
      cache: {
        ttl: CACHE_TTL.STANDARD, // 5 minutes cache for messageRequest reads
    ...
    const messageRequestResult = await this.query.findById(
      actor,
      params.id,
      repositoryOptions,
    );

    if (!isOk(messageRequestResult)) {
      const contextError = withContext(messageRequestResult.error, {
        correlationId,
        userId: params.user.sub,
        operation: 'get_message_request',
        id: params.id,
    ...
    // Step 5: Handle Option<DetailMessageRequestResponse> safely - query port already returns DTOs (proper CQRS)
    const messageRequestDto = Option.isNone(messageRequestResult.value)
      ? null
      : messageRequestResult.value.value;

    // Step 5.1: Return 404 error if messageRequest not found
    if (!messageRequestDto) {
      const notFoundError = withContext(
        MessageRequestErrors.MESSAGE_REQUEST_NOT_FOUND,
        {
          id: params.id,
