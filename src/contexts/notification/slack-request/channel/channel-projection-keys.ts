...
export class ChannelProjectionKeys {
   ...
   * Example: slack-request.channel.v1
   */
   ...
   * Example: $ce-slack-request.channel.v1
   */
   ...
   * Example: slack-request.channel.v1-tenant-12345
   */
   ...
   * Format: notification.slack:v1:{tenant}:channel:{channelId}
   */
  static getRedisChannelKey(tenant: string, channelId: string): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:{${tenant}}:channel:${channelId}`;
  }

  /**
   * Get Redis key for channel index with cluster-safe hash tags
   * Format: notification.slack:v1:{tenant}:idx:channel:list
   */
  static getRedisChannelIndexKey(tenant: string): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:{${tenant}}:idx:channel:list`;
  }

  /**
   * Get Redis key for category-based channel index with cluster-safe hash tags
   * Format: notification.slack:v1:{tenant}:idx:channel:by-category:{category}
   */
  static getRedisCategoryIndexKey(tenant: string, category: string): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:{${tenant}}:idx:channel:by-category:${category}`;
  }

  /**
   * Get Redis key pattern for all channels in a tenant
   * Format: notification.slack:v1:{tenant}:channel:*
   */
  static getRedisTenantChannelPattern(tenant: string): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:{${tenant}}:channel:*`;
  }
   ...
   * Format: notification.slack:pp:ver:{tenant}:channel:{channelId}
   */
  static getNamespacedVersionHintKey(
    tenant: string,
    channelId: string,
  ): string {
    return `notification.slack:pp:ver:{${tenant}}:channel:${channelId}`;
  }
