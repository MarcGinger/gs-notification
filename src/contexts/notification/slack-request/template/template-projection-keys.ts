...
export class TemplateProjectionKeys {
   ...
   * Example: slack-request.template.v1
   */
   ...
   * Example: $ce-slack-request.template.v1
   */
   ...
   * Example: slack-request.template.v1-tenant-12345
   */
   ...
   * Format: notification.slack:v1:{tenant}:template:{templateId}
   */
  static getRedisTemplateKey(tenant: string, templateId: string): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:{${tenant}}:template:${templateId}`;
  }

  /**
   * Get Redis key for template index with cluster-safe hash tags
   * Format: notification.slack:v1:{tenant}:idx:template:list
   */
  static getRedisTemplateIndexKey(tenant: string): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:{${tenant}}:idx:template:list`;
  }

  /**
   * Get Redis key for category-based template index with cluster-safe hash tags
   * Format: notification.slack:v1:{tenant}:idx:template:by-category:{category}
   */
  static getRedisCategoryIndexKey(tenant: string, category: string): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:{${tenant}}:idx:template:by-category:${category}`;
  }

  /**
   * Get Redis key pattern for all templates in a tenant
   * Format: notification.slack:v1:{tenant}:template:*
   */
  static getRedisTenantTemplatePattern(tenant: string): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:{${tenant}}:template:*`;
  }
   ...
   * Format: notification.slack:pp:ver:{tenant}:template:{templateId}
   */
  static getNamespacedVersionHintKey(
    tenant: string,
    templateId: string,
  ): string {
    return `notification.slack:pp:ver:{${tenant}}:template:${templateId}`;
  }
