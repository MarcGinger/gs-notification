...
/**
 * FullMessage Foreign Key Validation Service
 *
 * Provides centralized validation for foreign key references used in FullMessage domain.
 * This service validates that referenced entities exist before FullMessage creation or updates.
 *
 * Benefits:
 ...
 */
@Injectable()
export class FullMessageForeignKeyValidatorService {
  constructor(
  ...
  async validateWorkspaceCode(
    ...
    if (Option.isNone(workspace)) {
      ...
      return err(
        withContext(FullMessageErrors.INVALID_WORKSPACE_CODE, {
          correlationId: context.correlationId,
          userId: context.userId,
          operation: context.operation,
          workspaceCode,
  ...
  async validateTemplateCode(
    ...
    if (Option.isNone(template)) {
      ...
      return err(
        withContext(FullMessageErrors.INVALID_TEMPLATE_CODE, {
          correlationId: context.correlationId,
          userId: context.userId,
          operation: context.operation,
          templateCode,
  ...
  async validateChannelCode(
    ...
    if (Option.isNone(channel)) {
      ...
      return err(
        withContext(FullMessageErrors.INVALID_CHANNEL_CODE, {
          correlationId: context.correlationId,
          userId: context.userId,
          operation: context.operation,
          channelCode,
  ...
  /**
   * Validates all foreign keys for fullMessage props
   * @param actor - User context for authorization
   * @param props - FullMessage properties containing foreign key fields
   * @param context - Validation context for logging and error handling
   * @param logger - Logger instance for structured logging
   * @returns Result indicating validation success or failure
   */
  async validateFullMessageForeignKeys(
    actor: ActorContext,
    props: {
  ...
  /**
   * Generic alias for validateFullMessageForeignKeys to match shared runner interface
  ...
  async validateForeignKeys(
    ...
    >
  > {
    return this.validateFullMessageForeignKeys(actor, props, context, logger);
  }
}
