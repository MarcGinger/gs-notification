...
/**
 * FullMessage documentation endpoints interface
 */
export interface FullMessageDocumentationUrls {
  fullMessage: string;
}

/**
 * FullMessage Documentation
 * This module handles the Swagger documentation for full_messages
 *
 */
export class FullMessageDocumentation {
  static setup(app: INestApplication, port: string | number): void {
    const config = new DocumentBuilder()
      ...
      .setTitle('📱 FullMessage Management API')
      .setDescription(
        `
...
`,
      ...
      .setVersion('1.0.0')
      .addTag('FullMessages', `Configuration for full_message table`);

    // Add dynamic server configuration
    SwaggerConfigUtil.addServers(config, port);

    const document = SwaggerModule.createDocument(app, config.build(), {
      include: [FullMessageModule],
      extraModels: [],
    });

    SwaggerModule.setup(
      'api/docs/notification/slack-request/full-message',
      app,
      document,
    );
  }
  ...
  /**
   * Setup all fullMessage-related documentation endpoints
   * This method centralizes the setup of all fullMessage module documentation
   */
  static setupAll(
    app: INestApplication,
    port: string | number,
  ): FullMessageDocumentationUrls {
    // Setup main fullMessage documentation
    FullMessageDocumentation.setup(app, port);

    // Setup sub-module documentation

    // Return all endpoint URLs
    return {
      fullMessage: FullMessageDocumentation.getEndpoint(port),
    };
  }

  static getEndpoint(port: string | number): string {
    return `${SwaggerConfigUtil.getServerUrl(port)}/api/docs/notification/slack-request/full-message`;
  }
}
