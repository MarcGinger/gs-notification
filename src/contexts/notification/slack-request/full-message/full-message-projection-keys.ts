...
/**
 * Domain value object for FullMessage projection keys
...
export class FullMessageProjectionKeys {
   ...
   * Example: $ce-slack-request.full-message.v1
   */
   ...
   * Example: slack-request.full-message.v1-tenant-12345
   */
  ...
  /**
   * Validate if stream name matches full-message pattern
   */
  static isFullMessageStream(streamName: string): boolean {
  ...
  static readonly PROJECTOR_NAME = 'full-message-projector';
  static readonly SUBSCRIPTION_GROUP = 'full-message-projection';

  // ⚠️ IMPORTANT: Cache optimization keys use different format
  // The CacheOptimizationUtils class uses notification.slack:pp:ver:{tenant}:full-message:{fullMessageId}
  ...
  /**
   * Get Redis key for full-message projection with cluster-safe hash tags
   * Format: notification.slack:v1:{tenant}:full-message:{fullMessageId}
   */
  static getRedisFullMessageKey(tenant: string, fullMessageId: string): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:{${tenant}}:full-message:${fullMessageId}`;
  }

  /**
   * Get Redis key for full-message index with cluster-safe hash tags
   * Format: notification.slack:v1:{tenant}:idx:full-message:list
   */
  static getRedisFullMessageIndexKey(tenant: string): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:{${tenant}}:idx:full-message:list`;
  }

  /**
   * Get Redis key for category-based full-message index with cluster-safe hash tags
   * Format: notification.slack:v1:{tenant}:idx:full-message:by-category:{category}
   */
  static getRedisCategoryIndexKey(tenant: string, category: string): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:{${tenant}}:idx:full-message:by-category:${category}`;
  }

  /**
   * Get Redis key pattern for all full-messages in a tenant
   * Format: notification.slack:v1:{tenant}:full-message:*
   */
  static getRedisTenantFullMessagePattern(tenant: string): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:{${tenant}}:full-message:*`;
  }

  /**
   * Get Redis key pattern for all full-message projections
   * Format: notification.slack:v1:*:full-message:*
   */
  static getRedisAllFullMessagesPattern(): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:*:full-message:*`;
  }
   ...
   * Format: notification.slack:pp:ver:{tenant}:full-message:{fullMessageId}
   */
  static getNamespacedVersionHintKey(
    tenant: string,
    fullMessageId: string,
  ): string {
    return `notification.slack:pp:ver:{${tenant}}:full-message:${fullMessageId}`;
  }
  ...
  static getCurrentKeyPatterns(): string[] {
    return [
      'notification.slack:v1:*:full-message:*', // Business data keys
      'notification.slack:pp:ver:*:full-message:*', // Namespaced version hints
