...
// FullMessage Projector - ESDB to Redis Projection
...
// ✅ Import full-messageion-ready shared utilities
...
import { FullMessageFieldValidatorUtil } from '../utilities/full-message-field-validator.util';
import { DetailFullMessageResponse } from '../../application/dtos';
import { FullMessageQueueService, SendFullMessageJob } from '../services';
import { IRedisIdempotencyService } from 'src/shared/infrastructure';
/**
 * FullMessage projector error catalog using shared error definitions
 */
const FullMessageProjectorErrors = createProjectorErrorCatalog(
  'FULL_MESSAGE_PROJECTOR',
  CommonProjectorErrorDefinitions,
);

/**
 * FullMessage row parameters for Redis projection operations
 *
 * Extends DetailFullMessageResponse DTO with projection-specific fields needed for event sourcing.
 *
 * Key Additions to DetailFullMessageResponse:
 ...
 */
interface FullMessageProjectionParams extends DetailFullMessageResponse {
...
/**
 * FullMessage Projector using Redis-based Shared Projection Infrastructure
 *
  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  constructor(
    ...
    private readonly queueService: FullMessageQueueService,
    @Inject('FullMessageIdempotencyService')
    private readonly idempotencyService: IRedisIdempotencyService,
  ) {
    super(
      FullMessageProjectionKeys.PROJECTOR_NAME,
      FullMessageProjectionKeys.SUBSCRIPTION_GROUP,
      baseLogger,
      checkpointStore,
    );

    // ✅ Initialize shared utilities for full-messageion-ready operations
    this.cacheOptimization = new CacheOptimizationUtils();
    this.pipelineBuilder = new RedisPipelineBuilder();

    Log.info(
      this.logger,
      'FullMessageProjector initialized with full-messageion-ready shared utilities',
      {
        method: 'constructor',
  ...
  /**
   * Start the projector using CatchUpRunner with full-messageion-ready utilities
   */
  onModuleInit(): void {
    Log.info(this.logger, 'Starting FullMessage Projector with CatchUpRunner', {
      method: 'onModuleInit',
      subscriptionGroup: this.subscriptionGroup,
    });
    ...
    try {
      ...
      const runOptions: RunOptions = {
        prefixes: [FullMessageProjectionKeys.getEventStoreStreamPrefix()],
        batchSize: 100,
      ...
      Log.info(this.logger, 'FullMessage Projector started successfully', {
        method: 'onModuleInit',
        status: 'running',
      });
    } catch (error) {
      ...
      Log.error(this.logger, 'Failed to start FullMessage Projector', {
        method: 'onModuleInit',
        error: e.message,
        stack: e.stack,
      });
  ...
  onModuleDestroy(): void {
    Log.info(this.logger, 'Stopping FullMessage Projector', {
    ...
    try {
      ...
      Log.info(this.logger, 'FullMessage Projector stopped successfully', {
        method: 'onModuleDestroy',
        status: 'stopped',
      });
    } catch (error) {
      const e = error as Error;
      Log.error(this.logger, 'Error stopping FullMessage Projector', {
        method: 'onModuleDestroy',
        error: e.message,
        stack: e.stack,
      });
  ...
  /**
   * Project individual event using full-messageion-ready EventPipelineProcessor
  ...
  private async projectEvent(
    ...
    try {
      // ✅ Extract full-message parameters using existing utility
      const params = this.extractFullMessageParams(event, 'project');
      ...
      throw new Error(
        withContext(FullMessageProjectorErrors.DATABASE_OPERATION_FAILED, {
          eventType: event.type,
          streamId: event.streamId,
   ...
   */
  private generateClusterSafeKeys(params: FullMessageProjectionParams): {
    entityKey: string;
    indexKey: string;
  } {
  ...
  /**
   * Extract full-message parameters from event data using FullMessageFieldValidatorUtil
   *
   * Uses FullMessageFieldValidatorUtil to create validated DetailFullMessageResponse for consistent
   * validation across repository and projector, and TenantExtractor for reliable tenant identification.
   */
  private extractFullMessageParams(
    event: ProjectionEvent,
    operation: string,
  ): FullMessageProjectionParams {
    try {
      ...
      // Use FullMessageFieldValidatorUtil to create validated fullMessage snapshot
      const fullMessageSnapshot =
        FullMessageFieldValidatorUtil.createFullMessageSnapshotFromEventData(
          eventData,
        );
      ...
      const e = error as Error;
      throw new Error(
        withContext(FullMessageProjectorErrors.INVALID_EVENT_DATA, {
          eventType: event.type,
          streamId: event.streamId,
          operation,
          originalError: e.message,
  ...
  private async dispatchJobsForEvent(
    event: ProjectionEvent,
    params: FullMessageProjectionParams,
  ): Promise<void> {
    try {
      ...
      const shouldDispatch =
        simpleEventType === 'FullMessageCreated' ||
        (simpleEventType === 'FullMessageUpdated' && status === 'queued');
   ...
   * Only dispatches on FullMessageCreated or status transitions to 'queued'
   *
  ...
  private async handleJobDispatchingByEventType(
    ...
    // Extract the simple event name from the full type (e.g., "NotificationSlackRequestFullMessageCreated.v1" -> "FullMessageCreated")
    ...
    switch (simpleEventType) {
      ...
      // Note: Removed FullMessageFailed auto-retry
  ...
  private async dispatchSendMessageJob(
    ...
    try {
      ...
      const enqueuResult: {
        ...
        jobId?: string;
        error?: string;
      } = await this.queueService.enqueueSimpleSendFullMessageJob(job, {
