...
/**
 * FullMessage Reader Repository - Redis Implementation
 *
 * Bounded Context: Notification/FullMessage
 * Handles basic read operations for FullMessage aggregates using Redis projections
 ...
 *
 * @domain Notification Context - FullMessage Reader Repository (Redis)
 * @layer Infrastructure
 * @pattern Repository Pattern + Redis Projector
 */
@Injectable()
export class FullMessageReaderRepository implements IFullMessageReader {
  ...
  constructor(
    ...
    this.loggingConfig = {
      ...
      component: 'FullMessageReaderRepository',
    };
    this.logger = componentLogger(baseLogger, this.loggingConfig.component);

    Log.info(
      this.logger,
      'FullMessageReaderRepository initialized with Redis backend',
      {
  ...
  /**
   * Parse Redis hash data into FullMessageSnapshotProps
   */
  private parseRedisHashToFullMessage(
    hashData: Record<string, string>,
  ): FullMessageSnapshotProps | null {
    try {
  ...
  async findById(
    ...
    const correlationId =
      ...
      CorrelationUtil.generateForOperation('full-message-find-by-id');
    ...
    RepositoryLoggingUtil.logAuthorizationSuccess(
      ...
      operation,
      logContext,
      { queryType: 'full-message_lookup', scope: 'redis_projection' },
    );
    ...
    try {
      ...
      // Get full-message hash from Redis
      ...
      // Parse Redis hash to FullMessageSnapshot
      const fullMessageSnapshot = this.parseRedisHashToFullMessage(hashData);

      if (!fullMessageSnapshot) {
        RepositoryLoggingUtil.logQueryMetrics(
          this.logger,
          operation,
          logContext,
      ...
      return ok(Option.some(fullMessageSnapshot));
    } catch (error) {
  ...
  async exists(
    ...
    const correlationId =
      options?.correlationId ??
      CorrelationUtil.generateForOperation('full-message-exists-check');
  ...
  async getVersion(
    ...
    const correlationId =
      options?.correlationId ??
      CorrelationUtil.generateForOperation('full-message-get-version');
  ...
  async getMinimal(
    ...
    const correlationId =
      options?.correlationId ??
      CorrelationUtil.generateForOperation('full-message-get-minimal');
