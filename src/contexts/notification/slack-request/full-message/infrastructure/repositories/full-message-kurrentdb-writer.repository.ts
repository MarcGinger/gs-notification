...
/**
 * FullMessage Writer Repository - Interface Segregation Principle Implementation
 *
 ...
 *
 * @domain Notification Context - FullMessage Writer Repository
 * @layer Infrastructure
 * @pattern Repository Pattern + Interface Segregation Principle
 */
export class FullMessageWriterRepository
  extends BaseWriterRepository
  implements IFullMessageWriter
{
  ...
  constructor(
    ...
    this.loggingConfig = {
      ...
      component: 'FullMessageWriterRepository',
    };
    this.logger = componentLogger(baseLogger, this.loggingConfig.component);
  }

  /**
   * Save a FullMessage aggregate (create or update) using EventStoreDB-first approach
  ...
  async save(
    ...
    const correlationId =
      CorrelationUtil.generateForOperation('full-message-save');
    ...
    const logContext = RepositoryLoggingUtil.createLogContext(
      ...
      actor,
      {
        aggregateId: fullMessage.id.value,
        currentVersion: fullMessage.version,
        eventCount: fullMessage.uncommittedEvents?.length ?? 0,
      },
    );
    ...
    const stream = this.buildStreamName(
      ...
      fullMessage.id.value,
    );
    const events = fullMessage.uncommittedEvents ?? [];
    ...
    if (events.length === 0) {
      ...
      const receipt: SaveReceipt = {
        ...
        eventCount: 0,
        newVersion: fullMessage.version,
        streamRevision:
          fullMessage.version === 0
            ? BigInt(-1)
            : BigInt(fullMessage.version - 1),
    ...
    const prevVersion = fullMessage.version - eventsToStore.length;
    ...
    const envelopes = this.toEnvelopes(
      ...
      eventsToStore,
      actor,
      'full-message-writer-repository',
    ...
    try {
      ...
      fullMessage.markEventsAsCommitted();
  ...
  async delete(
    ...
    const correlationId =
      ...
      CorrelationUtil.generateForOperation('full-message-delete');
    ...
    const eventMetadata: EventMetadata = {
      ...
      service: 'full-message-writer-repository',
      timestampIso: this.clock.nowIso(),
      eventVersion: '1.0',
    ...
    try {
      Log.debug(
        this.logger,
        'Appending full-message deletion event to EventStoreDB',
        {
          ...logContext,
          stream,
          fullMessageCode: code.value,
