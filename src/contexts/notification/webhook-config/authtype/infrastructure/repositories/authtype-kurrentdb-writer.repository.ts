 ...
 *
 * @domain Notification Context - Authtype Writer Repository
 * @layer Infrastructure
 * @pattern Repository Pattern + Interface Segregation Principle
 */
export class AuthtypeWriterRepository
  extends BaseWriterRepository
  implements IAuthtypeWriter
{
  ...
  /**
   * Save a Authtype aggregate (create or update) using EventStoreDB-first approach
  ...
  async save(
    ...
    const stream = this.buildStreamName(
      actor.tenant,
      AuthtypeProjectionKeys.getEventStoreStreamPrefix(),
      authtype.id.value,
    );
    ...
    if (events.length === 0) {
      ...
      const receipt: SaveReceipt = {
        ...
        streamRevision:
          authtype.version === 0 ? BigInt(-1) : BigInt(authtype.version - 1),
  ...
  /**
   * Delete a Authtype by its unique identifier using EventStoreDB-first approach
   * @param actor - The authenticated user context
  ...
  async delete(
    ...
    const eventMetadata: EventMetadata = {
      ...
      service: 'authtype-writer-repository',
      timestampIso: this.clock.nowIso(),
      eventVersion: '1.0',
    ...
    // Create proper AuthtypeDeletedEvent
    const deletedEvent = AuthtypeDeletedEvent.create(
      {
        id: id.value,
    ...
    try {
      Log.debug(
        this.logger,
        'Appending authtype deletion event to EventStoreDB',
        {
          ...logContext,
          stream,
          authtypeId: id.value,
      ...
      RepositoryLoggingUtil.logQueryMetrics(
        ...
        logContext,
        {
          ...
          sampleData: {
            ...
            authtypeId: id.value,
            streamRevision: appendResult?.nextExpectedRevision?.toString(),
            eventType: 'AuthtypeDeletedEvent',
          },
        },
      );
