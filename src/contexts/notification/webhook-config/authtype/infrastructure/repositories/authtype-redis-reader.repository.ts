...
/**
 * Authtype Reader Repository - Redis Implementation
 *
 * Bounded Context: Notification/Authtype
 * Handles basic read operations for Authtype aggregates using Redis projections
 ...
 *
 * @domain Notification Context - Authtype Reader Repository (Redis)
...
export class AuthtypeReaderRepository implements IAuthtypeReader {
  ...
  constructor(
    ...
    Log.info(
      this.logger,
      'AuthtypeReaderRepository initialized with Redis backend',
      {
  ...
  /**
   * Parse Redis hash data into AuthtypeSnapshotProps
   */
  private parseRedisHashToAuthtype(
    hashData: Record<string, string>,
  ): AuthtypeSnapshotProps | null {
    try {
      ...
      const type = hashData.type as AuthtypeSnapshotProps['type'];

      // Type assertion for signatureAlgorithm - cached data should already be validated
      const signatureAlgorithm =
        hashData.signatureAlgorithm as AuthtypeSnapshotProps['signatureAlgorithm'];
  ...
  async findById(
    ...
    RepositoryLoggingUtil.logAuthorizationSuccess(
      ...
      operation,
      logContext,
      { queryType: 'authtype_lookup', scope: 'redis_projection' },
    );

    try {
      // Generate cluster-safe Redis key
      const redisKey = this.generateAuthtypeKey(actor.tenant, id.value);

      Log.debug(this.logger, 'Finding authtype by ID in Redis', {
        ...logContext,
        queryDetails: {
          scope: 'redis_hash',
      ...
      // Parse Redis hash to AuthtypeSnapshot
      const authtypeSnapshot = this.parseRedisHashToAuthtype(hashData);
      ...
      return ok(Option.some(authtypeSnapshot));
    } catch (error) {
  ...
  async exists(
    ...
    try {
      ...
      Log.debug(this.logger, 'Checking authtype existence in Redis', {
        ...logContext,
        queryDetails: {
          scope: 'redis_hash',
          method: 'redis.exists',
  ...
  async getVersion(
    ...
    try {
      ...
      Log.debug(this.logger, 'Getting authtype version from Redis', {
        ...logContext,
        queryDetails: {
          scope: 'redis_hash',
          method: 'redis.hmget',
  ...
  async getMinimal(
    ...
    try {
      ...
      Log.debug(this.logger, 'Getting minimal authtype data from Redis', {
        ...logContext,
        queryDetails: {
          scope: 'redis_hash',
          method: 'redis.hmget',
