...
// Authtype Projector - ESDB to In-Memory Projection
...
const AuthtypeProjectorErrors = createProjectorErrorCatalog(
  'AUTHTYPE_PROJECTOR_INMEMORY',
  CommonProjectorErrorDefinitions,
);

/**
 * Extracted authtype parameters from event data
 */
interface ExtractedAuthtypeParams extends DetailAuthtypeResponse {
  tenant: string;
  version?: number;
}

/**
 * Extended authtype projection with projector-specific metadata
 */
interface InMemoryAuthtypeProjection extends AuthtypeProjection {
...
/**
 * Authtype Projector using In-Memory Architecture
 *
 * Perfect for:
  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  private shadowCallback?: (
    key: string,
    primaryResult: InMemoryAuthtypeProjection,
    shadowResult?: InMemoryAuthtypeProjection,
  ...
  constructor(
    ...
    super(
      'authtype-projector-inmemory',
      'authtype-projection-inmemory',
      baseLogger,
      inMemoryCheckpointStore,
    );
    ...
    Log.info(
      this.logger,
      'AuthtypeProjectorInMemory initialized using shared authtype store',
      {
        method: 'constructor',
  ...
  enableShadowMode(
    callback: (
      key: string,
      inMemoryProjection: InMemoryAuthtypeProjection,
   ...
   */
  loadSnapshot(snapshot: Map<string, InMemoryAuthtypeProjection>): void {
    authtypeStore.clear();

    for (const [key, projection] of snapshot) {
      const [tenant, , id] = key.split(':');
      authtypeStore.set(tenant, id.toString(), projection);
    }
   ...
   */
  createSnapshot(): Map<string, InMemoryAuthtypeProjection> {
    const snapshot = new Map<string, InMemoryAuthtypeProjection>();

    for (const [key, projection] of authtypeStore.getAll()) {
      // Add projector-specific metadata to base projection
      const extendedProjection: InMemoryAuthtypeProjection = {
        ...projection,
   ...
   */
  getProjection(key: string): InMemoryAuthtypeProjection | undefined {
    const [tenant, , id] = key.split(':');
    const projection = authtypeStore.get(tenant, id.toString());
   ...
   */
  getAllProjections(): Array<[string, InMemoryAuthtypeProjection]> {
    return authtypeStore.getAll().map(([key, projection]) => [
      key,
      {
        ...projection,
        projectionTimestamp: new Date(),
  ...
  onModuleInit(): void {
    Log.info(
      this.logger,
      'Starting Authtype Projector (In-Memory) with CatchUpRunner',
      {
        method: 'onModuleInit',
        subscriptionGroup: this.subscriptionGroup,
        architecture: 'in-memory',
        maxCacheSize: 5000, // From authtype store
      },
    );
    ...
    try {
      ...
      Log.info(
        this.logger,
        'Authtype Projector (In-Memory) started successfully',
        {
          method: 'onModuleInit',
          status: 'running',
      ...
      Log.error(this.logger, 'Failed to start Authtype Projector (In-Memory)', {
        method: 'onModuleInit',
        error: e.message,
        stack: e.stack,
      });
  ...
  onModuleDestroy(): void {
    Log.info(this.logger, 'Stopping Authtype Projector (In-Memory)', {
    ...
    try {
      ...
      Log.info(
        this.logger,
        'Authtype Projector (In-Memory) stopped successfully',
        {
          method: 'onModuleDestroy',
          status: 'stopped',
          totalEventsProcessed: this.metrics.getMetrics().totalEvents,
          finalCacheSize: authtypeStore.size(),
        },
      );
    } catch (error) {
      const e = error as Error;
      Log.error(this.logger, 'Error stopping Authtype Projector (In-Memory)', {
        method: 'onModuleDestroy',
        error: e.message,
        stack: e.stack,
      });
  ...
  private projectEvent(event: ProjectionEvent): void {
    ...
    try {
      ...
      // Store in shared authtype store
      authtypeStore.set(tenant, params.id.toString(), projection);
  ...
  private applyEventToMemory(
    event: ProjectionEvent,
    params: ExtractedAuthtypeParams,
  ): InMemoryAuthtypeProjection {
    ...
    const eventTs =
      hasTimestamp(event) && event.timestamp
        ...
        : this.clock.now();
    const existing = authtypeStore.get(params.tenant, params.id.toString());
    ...
    switch (event.type) {
      case 'NotificationWebhookConfigAuthtypeCreated.v1':
        return this.handleAuthtypeCreated(params, event, eventTs);
      case 'NotificationWebhookConfigAuthtypeUpdated.v1':
        return this.handleAuthtypeUpdated(params, event, eventTs);
      case 'NotificationWebhookConfigAuthtypeDeleted.v1':
        return this.handleAuthtypeDeleted(params, event, eventTs);
      default:
  ...
  /**
   * Handle AuthtypeCreated event
   */
  private handleAuthtypeCreated(
    params: ExtractedAuthtypeParams,
    event: ProjectionEvent,
    timestamp: Date,
  ): InMemoryAuthtypeProjection {
  ...
  /**
   * Handle AuthtypeUpdated event
   */
  private handleAuthtypeUpdated(
    params: ExtractedAuthtypeParams,
    event: ProjectionEvent,
    timestamp: Date,
  ): InMemoryAuthtypeProjection {
  ...
  /**
   * Handle AuthtypeDeleted event
   */
  private handleAuthtypeDeleted(
    params: ExtractedAuthtypeParams,
    event: ProjectionEvent,
    timestamp: Date,
  ): InMemoryAuthtypeProjection {
   ...
   */
  private buildProjectionKey(tenant: string, id: number): string {
    return `${tenant}:authtype:${id.toString()}`;
  }
  ...
  private extractAuthtypeParams(
    ...
    try {
      ...
      // Use AuthtypeFieldValidatorUtil to create validated initiation fee snapshot
  ...
  private updateMetrics(): void {
    this.metrics.updateEntityCount(authtypeStore.size());
  ...
  private estimateMemoryUsage(): number {
    ...
    return authtypeStore.size() * 1024;
  }
}
