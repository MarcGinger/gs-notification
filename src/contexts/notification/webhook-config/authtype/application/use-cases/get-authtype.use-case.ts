...
// Application layer
import { IAuthtypeQuery, AUTHTYPE_QUERY_TOKEN } from '../ports';
import { AuthtypeAuthorizationAdapter } from '../services';
import { DetailAuthtypeResponse } from '../dtos';
import { IGetAuthtypeUseCase } from './contracts';
import { AuthtypeErrors } from '../../domain/errors';

/**
 * Get Authtype Use Case - CQRS Read Side Implementation
 *
 ...
 * The query port returns read-optimized DetailAuthtypeResponse DTOs directly.
 */
...
/**
 * ✅ Enhanced Get Authtype Use Case with Modern Security and Compliance
 *
 * Features:
...
@Injectable()
export class GetAuthtypeUseCase implements IGetAuthtypeUseCase {
  private readonly logger: Logger;
  private readonly loggingConfig: UseCaseLoggingConfig;

  constructor(
    @Inject(AUTHTYPE_QUERY_TOKEN)
    private readonly query: IAuthtypeQuery,
  ...
  async execute(params: {
    ...
    const repositoryOptions: RepositoryOptions = {
      cache: {
        ttl: CACHE_TTL.STANDARD, // 5 minutes cache for authtype reads
    ...
    const authtypeResult = await this.query.findById(
      actor,
      params.id,
      repositoryOptions,
    );

    if (!isOk(authtypeResult)) {
      const contextError = withContext(authtypeResult.error, {
        correlationId,
        userId: params.user.sub,
        operation: 'get_authtype',
        id: params.id,
    ...
    // Step 5: Handle Option<DetailAuthtypeResponse> safely - query port already returns DTOs (proper CQRS)
    const authtypeDto = Option.isNone(authtypeResult.value)
      ? null
      : authtypeResult.value.value;

    // Step 5.1: Return 404 error if authtype not found
    if (!authtypeDto) {
      const notFoundError = withContext(AuthtypeErrors.AUTHTYPE_NOT_FOUND, {
        id: params.id,
        webhookId: '', // Default values for required fields
