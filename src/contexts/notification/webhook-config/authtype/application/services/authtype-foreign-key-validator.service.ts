...
/**
 * Authtype Foreign Key Validation Service
 *
 * Provides centralized validation for foreign key references used in Authtype domain.
 * This service validates that referenced entities exist before Authtype creation or updates.
 *
 * Benefits:
 ...
 */
@Injectable()
export class AuthtypeForeignKeyValidatorService {
  constructor(
  ...
  async validateWebhookId(
    ...
    if (Option.isNone(webhook)) {
      ...
      return err(
        withContext(AuthtypeErrors.INVALID_WEBHOOK_ID, {
          correlationId: context.correlationId,
          userId: context.userId,
          operation: context.operation,
          webhookId,
  ...
  /**
   * Validates all foreign keys for authtype props
   * @param actor - User context for authorization
   * @param props - Authtype properties containing foreign key fields
   * @param context - Validation context for logging and error handling
   * @param logger - Logger instance for structured logging
   * @returns Result indicating validation success or failure
   */
  async validateAuthtypeForeignKeys(
    actor: ActorContext,
    props: {
      webhookId?: string;
    },
  ...
  /**
   * Generic alias for validateAuthtypeForeignKeys to match shared runner interface
  ...
  async validateForeignKeys(
    ...
    >
  > {
    return this.validateAuthtypeForeignKeys(actor, props, context, logger);
  }
}
