...
import {
  TestSecurityId,
  createTestSecurityCreatedAt,
  createTestSecurityUpdatedAt,
  createdAtNow,
  updatedAtNow,
  validateMonotonicTime,
  bumpVersion,
  initialVersion,
  createTestSecurityVersion,
  TestSecurityVersion,
} from '../value-objects';
import { TestSecurityCreatedEvent, TestSecurityUpdatedEvent } from '../events';
import { TestSecurityErrors } from '../errors';
import { TestSecurityDomainState } from '../state';

/**
 * Domain Aggregate Root: TestSecurity
 *
 * Represents the TestSecurity aggregate root in the notification domain.
 * Encapsulates testSecurity business logic, state management, and domain invariants.
 *
 ...
 *
 * @domain Notification Context - TestSecurity Aggregate
 * @layer Domain Aggregates
 */

/**
 * TestSecurity Aggregate Root
 *
 * Central aggregate root that coordinates testSecurity business operations.
 * Manages consistency across the aggregate boundary and publishes domain events.
 */
export class TestSecurityAggregate extends AggregateRootBase {
  private _entity: TestSecurityEntity;
  private readonly clock: Clock;
  private readonly eventMetadata: EventMetadata;

  private constructor(
    entity: TestSecurityEntity,
    clock: Clock,
  ...
  /**
   * Factory method to create a new TestSecurity aggregate
   *
   * @param props - TestSecurity entity properties (should include createdAt, version)
   * @param clock - Clock instance for time operations
   * @param eventMetadata - Event metadata for domain events
   * @returns Result containing TestSecurityAggregate or DomainError
   */
  public static create(
    ...
    const createdAtResult = props.createdAt
      ? createTestSecurityCreatedAt(props.createdAt.value) // Extract Date from existing VO
      : createdAtNow(clock);
    ...
    const updatedAtResult = props.updatedAt
      ? createTestSecurityUpdatedAt(props.updatedAt.value) // Extract Date from existing VO
      : updatedAtNow(clock);
    ...
    const versionResult =
      props.version !== undefined
        ? TestSecurityVersion.create(props.version.value) // Extract number from existing VO
        : initialVersion();
    ...
    const entityResult = TestSecurityEntity.create(entityProps);
    if (!entityResult.ok) {
      return err(entityResult.error);
    }

    // Create the aggregate
    const aggregate = new TestSecurityAggregate(
      entityResult.value,
      clock,
      eventMetadata,
    );

    // Create typed TestSecurityCreatedEvent with only business data
    const createdEvent = TestSecurityCreatedEvent.create({
  ...
  public static reconstitute(
    ...
    const aggregate = new TestSecurityAggregate(entity, clock, eventMetadata);
  ...
  protected when(event: DomainEvent): void {
    switch (event.type) {
      ...
      case 'NotificationWebhookConfigTestSecurityUpdated.v1': {
        ...
        const entityResult = TestSecurityEntity.fromSnapshot({
          id: d.id,
          name: d.name,
  ...
  protected applySnapshot(
    ...
    const entityResult = TestSecurityEntity.fromSnapshot(snapshot);

    if (!entityResult.ok) {
      return err({
        code: 'TEST_SECURITY.SNAPSHOT_APPLICATION_FAILED',
   ...
   */
  public createSnapshot(): TestSecuritySnapshotProps {
   ...
   */
  public toDomainState(): TestSecurityDomainState {
   ...
   */
  public get entity(): TestSecurityEntity {
    return this._entity;
  }
  ...
  private detectChanges(
    validatedFields: ValidatedTestSecurityUpdateFields,
  ): boolean {
  ...
  public updateBatch(
    ...
    if (
      ...
      return err({
        ...TestSecurityErrors.CONCURRENCY_CONFLICT,
        context: {
    ...
    const currentVersionResult = createTestSecurityVersion(before.version);
    ...
    const updatedEvent = TestSecurityUpdatedEvent.create({
  ...
  /**
   * Delete the testSecurity entity (business operation with event publishing)
   *
   * @returns Result indicating success or failure
   */
   ...
   */
  public sameAs(other: TestSecurityAggregate): boolean {
