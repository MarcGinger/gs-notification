...
// TestSecurity Projector - ESDB to In-Memory Projection
...
import { TestSecurityFieldValidatorUtil } from '../utilities/test-security-field-validator.util';

/**
 * TestSecurity projector error catalog using shared error definitions
 */
const TestSecurityProjectorErrors = createProjectorErrorCatalog(
  'TEST_SECURITY_PROJECTOR_INMEMORY',
  CommonProjectorErrorDefinitions,
);

/**
 * Extracted testSecurity parameters from event data
 */
interface ExtractedTestSecurityParams extends DetailTestSecurityResponse {
  tenant: string;
  version?: number;
}

/**
 * Extended testSecurity projection with projector-specific metadata
 */
interface InMemoryTestSecurityProjection extends TestSecurityProjection {
...
/**
 * TestSecurity Projector using In-Memory Architecture
 *
 * Perfect for:
 ...
 */
@Injectable()
export class TestSecurityProjector
  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  private shadowCallback?: (
    key: string,
    primaryResult: InMemoryTestSecurityProjection,
    shadowResult?: InMemoryTestSecurityProjection,
  ...
  constructor(
    ...
    super(
      'test-security-projector-inmemory',
      'test-security-projection-inmemory',
      baseLogger,
      inMemoryCheckpointStore,
    );
    ...
    Log.info(
      this.logger,
      'TestSecurityProjectorInMemory initialized using shared testSecurity store',
      {
        method: 'constructor',
  ...
  enableShadowMode(
    callback: (
      key: string,
      inMemoryProjection: InMemoryTestSecurityProjection,
   ...
   */
  loadSnapshot(snapshot: Map<string, InMemoryTestSecurityProjection>): void {
    testSecurityStore.clear();

    for (const [key, projection] of snapshot) {
      const [tenant, , id] = key.split(':');
      testSecurityStore.set(tenant, id, projection);
    }
   ...
   */
  createSnapshot(): Map<string, InMemoryTestSecurityProjection> {
    const snapshot = new Map<string, InMemoryTestSecurityProjection>();

    for (const [key, projection] of testSecurityStore.getAll()) {
      // Add projector-specific metadata to base projection
      const extendedProjection: InMemoryTestSecurityProjection = {
        ...projection,
   ...
   */
  getProjection(key: string): InMemoryTestSecurityProjection | undefined {
    const [tenant, , id] = key.split(':');
    const projection = testSecurityStore.get(tenant, id);
   ...
   */
  getAllProjections(): Array<[string, InMemoryTestSecurityProjection]> {
    return testSecurityStore.getAll().map(([key, projection]) => [
      key,
      {
        ...projection,
        projectionTimestamp: new Date(),
  ...
  onModuleInit(): void {
    Log.info(
      this.logger,
      'Starting TestSecurity Projector (In-Memory) with CatchUpRunner',
      {
        method: 'onModuleInit',
        subscriptionGroup: this.subscriptionGroup,
        architecture: 'in-memory',
        maxCacheSize: 5000, // From testSecurity store
      },
    );

    try {
      const runOptions: RunOptions = {
        prefixes: [TestSecurityProjectionKeys.getEventStoreStreamPrefix()],
      ...
      Log.info(
        this.logger,
        'TestSecurity Projector (In-Memory) started successfully',
        {
          method: 'onModuleInit',
          status: 'running',
      ...
      Log.error(
        this.logger,
        'Failed to start TestSecurity Projector (In-Memory)',
        {
          method: 'onModuleInit',
          error: e.message,
          stack: e.stack,
  ...
  onModuleDestroy(): void {
    Log.info(this.logger, 'Stopping TestSecurity Projector (In-Memory)', {
    ...
    try {
      ...
      Log.info(
        this.logger,
        'TestSecurity Projector (In-Memory) stopped successfully',
        {
          method: 'onModuleDestroy',
          status: 'stopped',
          totalEventsProcessed: this.metrics.getMetrics().totalEvents,
          finalCacheSize: testSecurityStore.size(),
        },
      );
    } catch (error) {
      const e = error as Error;
      Log.error(
        this.logger,
        'Error stopping TestSecurity Projector (In-Memory)',
        {
          method: 'onModuleDestroy',
          error: e.message,
          stack: e.stack,
  ...
  private projectEvent(event: ProjectionEvent): void {
    ...
    try {
      const params = this.extractTestSecurityParams(event, 'project');
      const key = this.buildProjectionKey(tenant, params.id);

      const projection = this.applyEventToMemory(event, params);

      // Store in shared testSecurity store
      testSecurityStore.set(tenant, params.id, projection);
      ...
      Log.debug(this.logger, 'Event projected to in-memory', {
        ...
        latencyMs,
        cacheSize: testSecurityStore.size(),
        outcome: 'applied',
      });
      ...
      throw new Error(
        withContext(TestSecurityProjectorErrors.DATABASE_OPERATION_FAILED, {
          eventType: event.type,
          streamId: event.streamId,
  ...
  private applyEventToMemory(
    event: ProjectionEvent,
    params: ExtractedTestSecurityParams,
  ): InMemoryTestSecurityProjection {
    ...
    const eventTs =
      hasTimestamp(event) && event.timestamp
        ...
        : this.clock.now();
    const existing = testSecurityStore.get(params.tenant, params.id);
    ...
    switch (event.type) {
      case 'NotificationWebhookConfigTestSecurityCreated.v1':
        return this.handleTestSecurityCreated(params, event, eventTs);
      case 'NotificationWebhookConfigTestSecurityUpdated.v1':
        return this.handleTestSecurityUpdated(params, event, eventTs);
      case 'NotificationWebhookConfigTestSecurityDeleted.v1':
        return this.handleTestSecurityDeleted(params, event, eventTs);
      default:
  ...
  /**
   * Handle TestSecurityCreated event
   */
  private handleTestSecurityCreated(
    params: ExtractedTestSecurityParams,
    event: ProjectionEvent,
    timestamp: Date,
  ): InMemoryTestSecurityProjection {
  ...
  /**
   * Handle TestSecurityUpdated event
   */
  private handleTestSecurityUpdated(
    params: ExtractedTestSecurityParams,
    event: ProjectionEvent,
    timestamp: Date,
  ): InMemoryTestSecurityProjection {
  ...
  /**
   * Handle TestSecurityDeleted event
   */
  private handleTestSecurityDeleted(
    params: ExtractedTestSecurityParams,
    event: ProjectionEvent,
    timestamp: Date,
  ): InMemoryTestSecurityProjection {
   ...
   */
  private buildProjectionKey(tenant: string, id: string): string {
    return `${tenant}:test-security:${id}`;
  }
  ...
  /**
   * Extract testSecurity parameters from event data using TestSecurityFieldValidatorUtil
   *
   * Uses TestSecurityFieldValidatorUtil to create validated DetailTestSecurityResponse for consistent
   * validation across repository and projector, and TenantExtractor for reliable tenant identification.
   */
  private extractTestSecurityParams(
    event: ProjectionEvent,
    operation: string,
  ): ExtractedTestSecurityParams {
    try {
      ...
      const e = error as Error;
      throw new Error(
        withContext(TestSecurityProjectorErrors.INVALID_EVENT_DATA, {
          eventType: event.type,
          streamId: event.streamId,
          operation,
          originalError: e.message,
  ...
  private updateMetrics(): void {
    this.metrics.updateEntityCount(testSecurityStore.size());
  ...
  private estimateMemoryUsage(): number {
    ...
    return testSecurityStore.size() * 1024;
  }
}
