...
import { ITestSecurityWriter } from '../../application/ports';
import {
  testSecurityStore,
...
/**
 * TestSecurity Writer Repository - In-Memory Implementation
 *
 * Handles write operations (create, update, delete) for TestSecurity aggregates using in-memory projector
 ...
 *
 * @domain Notification Context - TestSecurity Writer Repository (In-Memory)
 * @layer Infrastructure
 * @pattern Repository Pattern + Interface Segregation Principle + In-Memory Store
 */
@Injectable()
export class TestSecurityWriterRepository implements ITestSecurityWriter {
  ...
  constructor(
    ...
    this.loggingConfig = {
      ...
      component: 'TestSecurityWriterRepository',
    };
  ...
  /**
   * Save a TestSecurity aggregate (create or update) using In-Memory store approach
  ...
  async save(
    ...
    const correlationId =
      CorrelationUtil.generateForOperation('test-security-save');

    const logContext = this.createLogContext(operation, correlationId, actor, {
      riskLevel,
      aggregateId: testSecurity.id.value,
      currentVersion: testSecurity.version,
      eventCount: testSecurity.uncommittedEvents?.length ?? 0,
    ...
    const events = testSecurity.uncommittedEvents ?? [];
    ...
    if (events.length === 0) {
      ...
      const receipt: SaveReceipt = {
        stream: `test-security-${testSecurity.id.value}`,
        aggregateId: testSecurity.id.value,
        tenant: actor.tenant,
        eventCount: 0,
        newVersion: testSecurity.version,
        streamRevision: BigInt(testSecurity.version),
    ...
    const prevVersion = testSecurity.version - events.length;
    ...
    try {
      Log.debug(this.logger, 'Saving test-security to shared store', {
        ...logContext,
        eventCount: events.length,
        prevVersion,
        queryDetails: {
          scope: 'shared_test-security_store',
          method: 'testSecurityStore.set',
        },
      });

      // Check if test-security already exists for version conflict detection
      const existingProjection = testSecurityStore.get(
        actor.tenant,
        testSecurity.id.value,
      );
      ...
      const projection: TestSecurityProjection = {
        // TestSecurityProps fields
        id: testSecurity.entity.id.value,
        name: testSecurity.entity.name.value,
        description: testSecurity.entity.description?.value,
        // BaseProjection fields
        tenant: actor.tenant,
        version: testSecurity.version,
        createdAt: testSecurity.entity.createdAt.value,
        updatedAt: this.clock.now(),
        deletedAt: undefined,
        lastStreamRevision: testSecurity.version.toString(),
      };

      // Save to in-memory store
      testSecurityStore.set(actor.tenant, testSecurity.id.value, projection);

      // Mark committed after successful store save
      testSecurity.markEventsAsCommitted();
  ...
  /**
   * Delete a TestSecurity by its unique identifier using In-Memory store approach
   * @param actor - The authenticated user context
  ...
  async delete(
    ...
    try {
      // Check if test-security exists
      if (!testSecurityStore.has(actor.tenant || 'default-tenant', id.value)) {
        ...
        RepositoryLoggingUtil.logOperationError(
          ...
          operation,
          logContext,
          new Error(`TestSecurity not found: ${id.value}`),
          'MEDIUM',
        );
      ...
      testSecurityStore.delete(actor.tenant || 'default-tenant', id.value);

      const receipt: SaveReceipt = {
        stream: `test-security-${id.value}`,
