...
/**
 * Domain value object for TestSecurity projection keys
...
export class TestSecurityProjectionKeys {
   ...
   * Example: $ce-webhook-config.test-security.v1
   */
   ...
   * Example: webhook-config.test-security.v1-tenant-12345
   */
  ...
  /**
   * Validate if stream name matches test-security pattern
   */
  static isTestSecurityStream(streamName: string): boolean {
  ...
  static readonly PROJECTOR_NAME = 'test-security-projector';
  static readonly SUBSCRIPTION_GROUP = 'test-security-projection';

  // ⚠️ IMPORTANT: Cache optimization keys use different format
  // The CacheOptimizationUtils class uses notification.slack:pp:ver:{tenant}:test-security:{testSecurityId}
  ...
  /**
   * Get Redis key for test-security projection with cluster-safe hash tags
   * Format: notification.slack:v1:{tenant}:test-security:{testSecurityId}
   */
  static getRedisTestSecurityKey(
    tenant: string,
    testSecurityId: string,
  ): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:{${tenant}}:test-security:${testSecurityId}`;
  }

  /**
   * Get Redis key for test-security index with cluster-safe hash tags
   * Format: notification.slack:v1:{tenant}:idx:test-security:list
   */
  static getRedisTestSecurityIndexKey(tenant: string): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:{${tenant}}:idx:test-security:list`;
  }

  /**
   * Get Redis key for category-based test-security index with cluster-safe hash tags
   * Format: notification.slack:v1:{tenant}:idx:test-security:by-category:{category}
   */
  static getRedisCategoryIndexKey(tenant: string, category: string): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:{${tenant}}:idx:test-security:by-category:${category}`;
  }

  /**
   * Get Redis key pattern for all test-securitys in a tenant
   * Format: notification.slack:v1:{tenant}:test-security:*
   */
  static getRedisTenantTestSecurityPattern(tenant: string): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:{${tenant}}:test-security:*`;
  }

  /**
   * Get Redis key pattern for all test-security projections
   * Format: notification.slack:v1:*:test-security:*
   */
  static getRedisAllTestSecuritiesPattern(): string {
    return `${this.REDIS_KEY_PREFIX}:${this.REDIS_VERSION}:*:test-security:*`;
  }
   ...
   * Format: notification.slack:pp:ver:{tenant}:test-security:{testSecurityId}
   */
  static getNamespacedVersionHintKey(
    tenant: string,
    testSecurityId: string,
  ): string {
    return `notification.slack:pp:ver:{${tenant}}:test-security:${testSecurityId}`;
  }
  ...
  static getCurrentKeyPatterns(): string[] {
    return [
      'notification.slack:v1:*:test-security:*', // Business data keys
      'notification.slack:pp:ver:*:test-security:*', // Namespaced version hints
