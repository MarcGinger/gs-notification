 ...
 * Handles workspace reference data lookups for FK validation in the Config context using Redis projections
 ...
 *
 * @domain Notification Context - Config Workspace Reader Repository (Redis)
  ...
  private parseRedisHashToWorkspace(
    ...
    try {
      ...
      return {
        ...
        method: hashData.method as WorkspaceMethodValue,
        headers,
        status: hashData.status as WorkspaceStatusValue,
      };
    } catch (error) {
      Log.error(
        this.logger,
    ...
    try {
      Log.debug(this.logger, 'Finding valid workspace ids in Redis', {
        ...
        queryDetails: {
          scope: 'redis_keys',
          method: 'redis.scan',
          pattern: `workspace-projector:{${actor.tenant}}:workspace:*`,
          clusterSafe: true,
        },
      });
