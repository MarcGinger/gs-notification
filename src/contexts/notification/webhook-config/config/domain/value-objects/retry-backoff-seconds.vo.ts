 ...
 */
export const ConfigRetryBackoffSeconds = createIntegerVO({
  name: 'RetryBackoffSeconds',
  required: true,

  errors: createIntegerVOErrors(
    ConfigErrors.INVALID_RETRY_BACKOFF_SECONDS,
    'RetryBackoffSeconds',
  ),
});

/** Public instance type for RetryBackoffSeconds */
export type ConfigRetryBackoffSeconds = IntegerVOInstance;

// Convenience creators
export const createConfigRetryBackoffSeconds = (v: number) =>
  ConfigRetryBackoffSeconds.create(v);
export const configRetryBackoffSecondsFrom = (v: unknown) =>
  ConfigRetryBackoffSeconds.from(v);
