...
export class WorkspaceQueryRepository implements IWorkspaceQuery {
  ...
  async findById(
    ...
    try {
      ...
      const detailResponse: DetailWorkspaceResponse = {
        ...
        webhookEventType: workspace.webhookEventType,
  ...
  private parseRedisHashToWorkspace(
    ...
    try {
      ...
      const method = hashData.method as DetailWorkspaceResponse['method'];

      // Type assertion for status - cached data should already be validated
      const status = hashData.status as DetailWorkspaceResponse['status'];
