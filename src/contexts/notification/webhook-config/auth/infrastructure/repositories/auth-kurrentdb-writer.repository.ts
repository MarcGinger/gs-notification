...
import { AuthDeletedEvent } from '../../domain/events';
import { IAuthWriter } from '../../application/ports';
import {
  EventEncryptionFactory,
  EncryptionConfig,
} from 'src/shared/infrastructure/encryption';
/**
 * Auth Writer Repository - Interface Segregation Principle Implementation
 *
 ...
 *
 * @domain Notification Context - Auth Writer Repository
 * @layer Infrastructure
 * @pattern Repository Pattern + Interface Segregation Principle
 */
export class AuthWriterRepository
  ...
  implements IAuthWriter
{
  ...
  constructor(
    ...
    this.loggingConfig = {
      ...
      component: 'AuthWriterRepository',
    };
    this.logger = componentLogger(baseLogger, this.loggingConfig.component);
  }

  /**
   * Save a Auth aggregate (create or update) using EventStoreDB-first approach
  ...
  async save(
    ...
    const correlationId = CorrelationUtil.generateForOperation('auth-save');
    ...
    const logContext = RepositoryLoggingUtil.createLogContext(
      ...
      actor,
      {
        aggregateId: auth.id.value,
        currentVersion: auth.version,
        eventCount: auth.uncommittedEvents?.length ?? 0,
      },
    );
    ...
    const stream = this.buildStreamName(
      actor.tenant,
      AuthProjectionKeys.getEventStoreStreamPrefix(),
      auth.id.value,
    );
    const events = auth.uncommittedEvents ?? [];
    ...
    if (events.length === 0) {
      ...
      const receipt: SaveReceipt = {
        ...
        eventCount: 0,
        newVersion: auth.version,
        streamRevision:
          auth.version === 0 ? BigInt(-1) : BigInt(auth.version - 1),
    ...
    const prevVersion = auth.version - eventsToStore.length;
  ...
  /**
   * Delete a Auth by its unique identifier using EventStoreDB-first approach
   * @param actor - The authenticated user context
  ...
  async delete(
    ...
    const correlationId =
      ...
      CorrelationUtil.generateForOperation('auth-delete');
    ...
    const eventMetadata: EventMetadata = {
      ...
      service: 'auth-writer-repository',
      timestampIso: this.clock.nowIso(),
      eventVersion: '1.0',
    ...
    // Create proper AuthDeletedEvent
    const deletedEvent = AuthDeletedEvent.create(
      {
    ...
    try {
      Log.debug(this.logger, 'Appending auth deletion event to EventStoreDB', {
        ...logContext,
        stream,
        authWebhookId: webhookId.value,
        expectedRevision: expectedRevision.toString(),
