...
// Auth Projector - ESDB to Redis Projection
...
// ✅ Import authion-ready shared utilities
...
import { AuthFieldValidatorUtil } from '../utilities/auth-field-validator.util';
import {
...
/**
 * Auth projector error catalog using shared error definitions
 */
const AuthProjectorErrors = createProjectorErrorCatalog(
  'AUTH_PROJECTOR',
  CommonProjectorErrorDefinitions,
);

/**
 * Auth projection parameters for unified processing
 */
interface AuthProjectionParams extends UnifiedProjectionParams {
...
/**
 * Auth Projector using Redis-based Shared Projection Infrastructure
 *
 ...
 *
 * Redis Data Structure (managed by AuthProjectionKeys domain value object):
 * - Auth Hash: `auth:projection:{tenant}:webhookId` - stores all auth fields
 * - Tenant Index: `auth:index:by_tenant:{tenant}` - sorted set by updated timestamp
 *
 * Key Features:
  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  private static readonly UNIFIED_CONFIG: UnifiedProjectionConfig = {
    ...
    getRedisEntityKey: (tenant: string, id: string) =>
      AuthProjectionKeys.getRedisAuthKey(tenant, id),
    getRedisIndexKey: (tenant: string) =>
      AuthProjectionKeys.getRedisAuthIndexKey(tenant),
    getEntityId: (params) => (params as AuthProjectionParams).webhookId,
  };
  ...
  constructor(
    ...
    super(
      AuthProjectionKeys.PROJECTOR_NAME,
      AuthProjectionKeys.SUBSCRIPTION_GROUP,
      baseLogger,
      checkpointStore,
    );
    ...
    Log.info(
      this.logger,
      'AuthProjector initialized with production-ready utilities',
      {
        method: 'constructor',
  ...
  /**
   * Start the projector using CatchUpRunner with authion-ready utilities
   */
  onModuleInit(): void {
    Log.info(
      this.logger,
      'Starting Auth Projector with production-ready features',
      {
        method: 'onModuleInit',
        subscriptionGroup: this.subscriptionGroup,
      },
    ...
    try {
      ...
      const runOptions: RunOptions = {
        prefixes: [AuthProjectionKeys.getEventStoreStreamPrefix()],
        batchSize: 100,
      ...
      Log.info(this.logger, 'Auth Projector started successfully', {
        method: 'onModuleInit',
        status: 'running',
      });
    } catch (error) {
      ...
      Log.error(this.logger, 'Failed to start Auth Projector', {
        method: 'onModuleInit',
        error: e.message,
        stack: e.stack,
      });
  ...
  onModuleDestroy(): void {
    Log.info(this.logger, 'Stopping Auth Projector', {
    ...
    try {
      ...
      Log.info(this.logger, 'Auth Projector stopped successfully', {
        method: 'onModuleDestroy',
        status: 'stopped',
      });
    } catch (error) {
      const e = error as Error;
      Log.error(this.logger, 'Error stopping Auth Projector', {
        method: 'onModuleDestroy',
        error: e.message,
        stack: e.stack,
      });
  ...
  private async projectEvent(
    ...
    try {
      ...
      const params = this.extractAuthParams(event, 'project');
      ...
      throw new Error(
        withContext(AuthProjectorErrors.DATABASE_OPERATION_FAILED, {
          eventType: event.type,
          streamId: event.streamId,
  ...
  /**
   * Extract auth parameters with enhanced error context
   *
   * Uses AuthFieldValidatorUtil to create validated DetailAuthResponse for consistent
  ...
  private extractAuthParams(
    ...
    try {
      ...
      // Use existing AuthFieldValidatorUtil for validation
      const authData =
        AuthFieldValidatorUtil.createAuthProjectorDataFromEventData(eventData);
      ...
      const e = error as Error;
      throw new Error(
        withContext(AuthProjectorErrors.INVALID_EVENT_DATA, {
          eventType: event.type,
          streamId: event.streamId,
          operation,
          originalError: e.message,
