...
/**
 * Token for injecting IAuthQuery port implementation
 */
export const AUTH_QUERY_TOKEN = 'IAuthQuery' as const;

/**
 * Auth Query Port - Interface Segregation Principle Implementation
 *
 ...
 *
 * @domain Notification Context - Auth Query Port
 * @layer Application Ports
 * @pattern Interface Segregation Principle
 */
export interface IAuthQuery {
  /**
   * Find a single Auth by its unique identifier (for CQRS query operations)
   ...
   * @returns A promise resolving to a Result containing the Auth or null if not found
   */
  findById(
    actor: ActorContext,
    webhookId: string,
