...
export class SecureTestFieldValidatorUtil {
  /**
   * Create projector data with SecureTest objects for Redis storage
   *
   * This method extracts SecureTest objects from event data and serializes them
   * as JSON strings for storage in Redis. The reader repository will deserialize
   * and resolve these SecureTest objects back to actual secret values.
   *
   * @param aggregateData - Raw event data from EventStore containing SecureTest objects
   * @returns Projector data object with SecureTest fields as JSON strings
   */
  static createSecureTestProjectorDataFromEventData(
    ...
    // Extract credentials for validation (they're handled via SecureTest below)
