...
// Application layer
import { ISecureTestQuery, SECURE_TEST_QUERY_TOKEN } from '../ports';
import { SecureTestAuthorizationAdapter } from '../services';
import { DetailSecureTestResponse } from '../dtos';
import { IGetSecureTestUseCase } from './contracts';
import { SecureTestErrors } from '../../domain/errors';

/**
 * Get SecureTest Use Case - CQRS Read Side Implementation
 *
 ...
 * The query port returns read-optimized DetailSecureTestResponse DTOs directly.
 */
...
/**
 * ✅ Enhanced Get SecureTest Use Case with Modern Security and Compliance
 *
 * Features:
...
@Injectable()
export class GetSecureTestUseCase implements IGetSecureTestUseCase {
  private readonly logger: Logger;
  private readonly loggingConfig: UseCaseLoggingConfig;

  constructor(
    @Inject(SECURE_TEST_QUERY_TOKEN)
    private readonly query: ISecureTestQuery,
    ...
    this.loggingConfig = {
      ...
      component: 'GetSecureTestUseCase',
  ...
  async execute(params: {
    ...
    const operation = 'get_secure_test';
    ...
    const authResult = await this.authorizationService.canReadResource(
      params.user.sub,
      params.id, // Using id as secureTest identifier
      correlationId,
      {
    ...
    if (
      ...
      const validationError = {
        code: 'SECURE_TEST.INVALID_THE_CODE' as const,
        title: 'Invalid secureTest id',
        category: 'validation' as const,
        context: {
          id: params.id,
          correlationId,
          userId: params.user.sub,
          operation: 'get_secureTest',
        },
      };
      UseCaseLoggingUtil.logOperationError(
        this.logger,
    ...
    const repositoryOptions: RepositoryOptions = {
      cache: {
        ttl: CACHE_TTL.STANDARD, // 5 minutes cache for secureTest reads
    ...
    const secureTestResult = await this.query.findById(
      actor,
      params.id,
      repositoryOptions,
    );

    if (!isOk(secureTestResult)) {
      const contextError = withContext(secureTestResult.error, {
        correlationId,
        userId: params.user.sub,
        operation: 'get_secure_test',
        id: params.id,
    ...
    // Step 5: Handle Option<DetailSecureTestResponse> safely - query port already returns DTOs (proper CQRS)
    const secureTestDto = Option.isNone(secureTestResult.value)
      ? null
      : secureTestResult.value.value;

    // Step 5.1: Return 404 error if secureTest not found
    if (!secureTestDto) {
      const notFoundError = withContext(
        SecureTestErrors.SECURE_TEST_NOT_FOUND,
        {
          id: params.id,
