...
import { CreateWebhookProps, UpsertWebhookProps } from '../../domain/props';
import { WebhookId } from '../../domain/value-objects';
import {
  createWebhookAggregateFromProps,
  updateWebhookAggregateFromSnapshot,
...
type WebhookSnapshot = Parameters<typeof updateWebhookAggregateFromSnapshot>[0];

@Injectable()
export class UpsertWebhookUseCase implements IUpsertWebhookUseCase {
  private readonly logger: Logger;
  private readonly loggingConfig: UseCaseLoggingConfig;

  constructor(
    @Inject(WEBHOOK_READER_TOKEN)
    private readonly webhookReader: IWebhookReader,
    @Inject(WEBHOOK_WRITER_TOKEN)
    private readonly webhookWriter: IWebhookWriter,
    ...
    this.loggingConfig = {
      ...
      component: 'UpsertWebhookUseCase',
  ...
  async execute(params: {
    ...
    const operation = 'upsert_webhook';
    ...
    // Note: UpsertWebhookProps approach provides cleaner validation semantics
    ...
    const codeR = WebhookId.create(command.id);
    ...
    const existsR = await this.webhookReader.exists(actor, codeR.value);
    ...
    const aggregateResult = await runUseCaseWithSecurity<
      UpsertWebhookCommand,
      WebhookAggregate,
      WebhookSnapshot
    >({
      serviceName: WebhookConfigServiceConstants.SERVICE_NAME,
      component: 'UpsertWebhookUseCase',
      operation: isCreate ? 'create_webhook' : 'update_webhook',
      source: 'webhook-config.application.upsert-webhook',
      command,
      logger: this.logger,
      clock: this.clock,
      ...
      repo: {
        save: async (
          ...
          const result = await this.webhookWriter.save(
            actor,
            webhook,
            expectedVersion,
          );
      ...
      propsMissingError: WebhookErrors.INVALID_WEBHOOK_DATA,
      ...
      loadExisting: async () => {
        if (!isCreate) {
          ...
          const foundR = await this.webhookReader.findById(actor, codeR.value);
          ...
          if (Option.isNone(foundR.value)) {
            return ok(undefined as unknown as WebhookSnapshot);
          }

          return ok(foundR.value.value);
        }
        // Return ok with undefined for create path; runDomain will create
        return Promise.resolve(ok(undefined as unknown as WebhookSnapshot));
      },
      ...
      runDomain: ({ existing, metadata, clock }) => {
        ...
        if (existing) {
          ...
          return updateWebhookAggregateFromSnapshot(
            existing,
            rawProps || {},
            enhancedMetadata,
            clock,
        ...
        const createProps = upsertProps as CreateWebhookProps;
        return createWebhookAggregateFromProps(
          createProps,
          enhancedMetadata,
          clock,
        );
    ...
    const dto = WebhookDtoAssembler.toDetailResponse(domainState);
