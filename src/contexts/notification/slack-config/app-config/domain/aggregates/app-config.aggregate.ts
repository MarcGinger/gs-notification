...
export class AppConfigAggregate extends AggregateRootBase {
  ...
  protected when(event: DomainEvent): void {
    switch (event.type) {
      ...
      case 'AppConfigUpdated': {
        ...
        const entityResult = AppConfigEntity.fromSnapshot(d.after);
        if (entityResult.ok) {
          this._entity = entityResult.value;
        }
        break;
  ...
  private buildChangeSet(
    ...
    const cs: AppConfigChangeSet = {};

    // Simple field comparisons
    compareField(before.workspaceId, after.workspaceId, 'workspaceId', cs);
    compareField(
      before.maxRetryAttempts,
      after.maxRetryAttempts,
      'maxRetryAttempts',
      cs,
    );
    compareField(
      before.retryBackoffSeconds,
      after.retryBackoffSeconds,
      'retryBackoffSeconds',
      cs,
    );
    compareField(
    ...
    compareField(
      before.loggingEnabled,
      after.loggingEnabled,
      'loggingEnabled',
      cs,
    );
    compareField(
      before.auditChannelId,
      after.auditChannelId,
      'auditChannelId',
      cs,
    );
    compareComplexField(before.metadata, after.metadata, 'metadata', cs);
  ...
  /**
   * Helper method to publish AppConfigUpdatedEvent with consistent structure
   *
  ...
  private publishUpdateEvent(
    before: AppConfigSnapshotProps,
    after: AppConfigSnapshotProps,
    changeSet: AppConfigChangeSet,
  ): void {
    // Create typed AppConfigUpdatedEvent with full payload
