...
import { IUpsertAppConfigUseCase } from './contracts';
...
@Injectable()
export class UpsertAppConfigUseCase implements IUpsertAppConfigUseCase {
  ...
  constructor(
    ...
    this.loggingConfig = {
      ...
      component: 'UpsertAppConfigUseCase',
      domain: 'slack-config',
  ...
  async execute(params: {
    ...
    const operation = 'upsert_app_config';
    ...
    // Note: UpsertAppConfigProps approach provides cleaner validation semantics
    ...
    const codeR = AppConfigCode.create(command.code);
    ...
    const existsR = await this.appConfigReader.exists(actor, codeR.value);
    ...
    >({
      serviceName: SlackConfigServiceConstants.SERVICE_NAME,
      component: 'UpsertAppConfigUseCase',
      operation: isCreate ? 'create_app_config' : 'update_app_config',
      source: 'app-config-config.application.upsert-app-config',
      command,
      logger: this.logger,
      clock: this.clock,
      ...
      loadExisting: async () => {
        if (!isCreate) {
          ...
          if (Option.isNone(foundR.value)) {
            return ok(undefined as unknown as AppConfigSnapshot);
          }

          return ok(foundR.value.value);
        }
        // Return ok with undefined for create path; runDomain will create
        return Promise.resolve(ok(undefined as unknown as AppConfigSnapshot));
      },
