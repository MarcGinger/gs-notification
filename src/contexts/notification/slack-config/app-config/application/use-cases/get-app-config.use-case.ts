 ...
 * The query port returns read-optimized DetailAppConfigResponse DTOs directly.
 */
...
/**
 * ✅ Enhanced Get AppConfig Use Case with Modern Security and Compliance
 *
 * Features:
...
@Injectable()
export class GetAppConfigUseCase implements IGetAppConfigUseCase {
  ...
  async execute(params: {
    ...
    const authResult = await this.authorizationService.canReadResource(
      params.user.sub,
      params.code, // Using code as appConfig identifier
      correlationId,
      {
    ...
    const repositoryOptions: RepositoryOptions = {
      cache: {
        ttl: CACHE_TTL.STANDARD, // 5 minutes cache for appConfig reads
    ...
    if (!isOk(appConfigResult)) {
      const contextError = withContext(appConfigResult.error, {
        correlationId,
        userId: params.user.sub,
        operation: 'get_app_config',
        code: params.code,
    ...
    // Step 5: Handle Option<DetailAppConfigResponse> safely - query port already returns DTOs (proper CQRS)
    const appConfigDto = Option.isNone(appConfigResult.value)
      ? null
