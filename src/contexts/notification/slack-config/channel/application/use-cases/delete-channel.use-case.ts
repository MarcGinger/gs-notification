...
import { deleteChannelAggregateFromSnapshot } from '../../domain/factories';
import { ChannelErrors } from '../../domain/errors';
import { IDeleteChannelUseCase } from './contracts';
...
type ChannelSnapshot = Parameters<typeof deleteChannelAggregateFromSnapshot>[0];

/**
 * Delete Channel Use Case
 *
 * Handles the secure deletion of existing channels with comprehensive
 ...
 *
 * @domain Notification Context - Channel Use Cases
...
export class DeleteChannelUseCase implements IDeleteChannelUseCase {
  ...
  constructor(
    ...
    this.loggingConfig = {
      ...
      component: 'DeleteChannelUseCase',
      domain: 'slack-config',
      entityType: 'channel',
    };
  ...
  /**
   * Execute the delete channel use case with enhanced security and compliance
   * @param command - The delete command containing channel identifier and context
  ...
  async execute(params: {
    ...
    const operation = 'delete_channel';
    ...
    try {
      ...
      >({
        ...
        source: 'slack-config.application.delete-channel',
        command,
        logger: this.logger,
        clock: this.clock,
        repo: {
          save: async (
            ...
            const saveResult = await this.channelWriter.save(
              actor,
              channel,
              expectedVersion,
            );
            ...
            const codeResult = ChannelId.from(rawProps.id);
            if (!codeResult.ok) return err(codeResult.error);

            const deleteResult = await this.channelWriter.delete(
              actor,
              codeResult.value,
            );
        ...
        loadExisting: async (actor, cmd) => {
          const codeR = ChannelId.from(rawProps.id);
        ...
        runDomain: ({ existing, metadata, clock }) => {
          ...
          return deleteChannelAggregateFromSnapshot(
            existing!,
            enhancedMetadata,
            clock,
          );
      ...
      const domainError = withContext(
        ChannelErrors.CANNOT_DELETE_CHANNEL_WITH_DEPENDENCIES,
        {
