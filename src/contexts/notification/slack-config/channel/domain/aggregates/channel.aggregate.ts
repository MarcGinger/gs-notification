...
export class ChannelAggregate extends AggregateRootBase {
  ...
  protected when(event: DomainEvent): void {
    switch (event.type) {
      ...
      case 'ChannelUpdated': {
        ...
        const entityResult = ChannelEntity.fromSnapshot(d.after);
        if (entityResult.ok) {
          this._entity = entityResult.value;
        }
        break;
  ...
  private buildChangeSet(
    ...
    const cs: ChannelChangeSet = {};

    // Simple field comparisons
    compareField(before.name, after.name, 'name', cs);
    compareField(before.workspaceId, after.workspaceId, 'workspaceId', cs);
    compareField(before.isPrivate, after.isPrivate, 'isPrivate', cs);
    compareField(before.isDm, after.isDm, 'isDm', cs);
    compareField(before.topic, after.topic, 'topic', cs);
    compareField(before.purpose, after.purpose, 'purpose', cs);
    compareComplexField(
      before.subscribedEvents,
      after.subscribedEvents,
      'subscribedEvents',
      cs,
    );
  ...
  /**
   * Helper method to publish ChannelUpdatedEvent with consistent structure
   *
  ...
  private publishUpdateEvent(
    before: ChannelSnapshotProps,
    after: ChannelSnapshotProps,
    changeSet: ChannelChangeSet,
  ): void {
    // Create typed ChannelUpdatedEvent with full payload
