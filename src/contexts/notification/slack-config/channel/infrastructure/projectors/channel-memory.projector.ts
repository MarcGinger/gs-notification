  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  loadSnapshot(snapshot: Map<string, InMemoryChannelProjection>): void {
    ...
    for (const [key, projection] of snapshot) {
      ...
      channelStore.set(tenantId, id, projection);
    }
  ...
  getProjection(key: string): InMemoryChannelProjection | undefined {
    const [tenantId, , id] = key.split(':');
    const projection = channelStore.get(tenantId, id);
  ...
  private projectEvent(event: ProjectionEvent): void {
    ...
    try {
      ...
      channelStore.set(tenant, params.id, projection);
  ...
  private applyEventToMemory(
    ...
    const eventTs =
      hasTimestamp(event) && event.timestamp
        ...
        : this.clock.now();
    const existing = channelStore.get(params.tenantId, params.id);
  ...
  private extractChannelParams(
    ...
    try {
      ...
      // Use ChannelFieldValidatorUtil to create validated initiation fee snapshot
