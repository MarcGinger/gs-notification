...
// Channel Projector - ESDB to In-Memory Projection
...
const ChannelProjectorErrors = createProjectorErrorCatalog(
  'CHANNEL_PROJECTOR_INMEMORY',
  CommonProjectorErrorDefinitions,
);

/**
 * Extracted channel parameters from event data
 */
interface ExtractedChannelParams extends DetailChannelResponse {
  tenant: string;
  version?: number;
}

/**
 * Extended channel projection with projector-specific metadata
 */
interface InMemoryChannelProjection extends ChannelProjection {
...
/**
 * Channel Projector using In-Memory Architecture
 *
 * Perfect for:
  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  private shadowCallback?: (
    key: string,
    primaryResult: InMemoryChannelProjection,
    shadowResult?: InMemoryChannelProjection,
  ...
  constructor(
    ...
    super(
      'channel-projector-inmemory',
      'channel-projection-inmemory',
      baseLogger,
      inMemoryCheckpointStore,
    );
    ...
    Log.info(
      this.logger,
      'ChannelProjectorInMemory initialized using shared channel store',
      {
        method: 'constructor',
  ...
  enableShadowMode(
    callback: (
      key: string,
      inMemoryProjection: InMemoryChannelProjection,
   ...
   */
  loadSnapshot(snapshot: Map<string, InMemoryChannelProjection>): void {
    channelStore.clear();

    for (const [key, projection] of snapshot) {
      const [tenant, , code] = key.split(':');
      channelStore.set(tenant, code, projection);
    }
   ...
   */
  createSnapshot(): Map<string, InMemoryChannelProjection> {
    const snapshot = new Map<string, InMemoryChannelProjection>();

    for (const [key, projection] of channelStore.getAll()) {
      // Add projector-specific metadata to base projection
      const extendedProjection: InMemoryChannelProjection = {
        ...projection,
   ...
   */
  getProjection(key: string): InMemoryChannelProjection | undefined {
    const [tenant, , code] = key.split(':');
    const projection = channelStore.get(tenant, code);
   ...
   */
  getAllProjections(): Array<[string, InMemoryChannelProjection]> {
    return channelStore.getAll().map(([key, projection]) => [
      key,
      {
        ...projection,
        projectionTimestamp: new Date(),
  ...
  onModuleInit(): void {
    Log.info(
      this.logger,
      'Starting Channel Projector (In-Memory) with CatchUpRunner',
      {
        method: 'onModuleInit',
        subscriptionGroup: this.subscriptionGroup,
        architecture: 'in-memory',
        maxCacheSize: 5000, // From channel store
      },
    );
    ...
    try {
      ...
      Log.info(
        this.logger,
        'Channel Projector (In-Memory) started successfully',
        {
          method: 'onModuleInit',
          status: 'running',
      ...
      Log.error(this.logger, 'Failed to start Channel Projector (In-Memory)', {
        method: 'onModuleInit',
        error: e.message,
        stack: e.stack,
      });
  ...
  onModuleDestroy(): void {
    Log.info(this.logger, 'Stopping Channel Projector (In-Memory)', {
    ...
    try {
      ...
      Log.info(
        this.logger,
        'Channel Projector (In-Memory) stopped successfully',
        {
          method: 'onModuleDestroy',
          status: 'stopped',
          totalEventsProcessed: this.metrics.getMetrics().totalEvents,
          finalCacheSize: channelStore.size(),
        },
      );
    } catch (error) {
      const e = error as Error;
      Log.error(this.logger, 'Error stopping Channel Projector (In-Memory)', {
        method: 'onModuleDestroy',
        error: e.message,
        stack: e.stack,
      });
  ...
  private projectEvent(event: ProjectionEvent): void {
    ...
    try {
      ...
      // Store in shared channel store
      channelStore.set(tenant, params.code, projection);
  ...
  private applyEventToMemory(
    event: ProjectionEvent,
    params: ExtractedChannelParams,
  ): InMemoryChannelProjection {
    ...
    const eventTs =
      hasTimestamp(event) && event.timestamp
        ...
        : this.clock.now();
    const existing = channelStore.get(params.tenant, params.code);
    ...
    switch (event.type) {
      case 'NotificationSlackConfigChannelCreated.v1':
        return this.handleChannelCreated(params, event, eventTs);
      case 'NotificationSlackConfigChannelUpdated.v1':
        return this.handleChannelUpdated(params, event, eventTs);
      case 'NotificationSlackConfigChannelDeleted.v1':
        return this.handleChannelDeleted(params, event, eventTs);
      default:
  ...
  private extractChannelParams(
    ...
    try {
      ...
      const subscribedEvents = channelSnapshot.subscribedEvents
        ? safeParseJSONArray(
            channelSnapshot.subscribedEvents,
            'subscribedEvents',
            isString,
          )
        : undefined;
  ...
  private updateMetrics(): void {
    this.metrics.updateEntityCount(channelStore.size());
