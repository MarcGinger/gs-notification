...
// Channel Projector - ESDB to PostgreSQL Projection
...
/**
 * Channel Projector using Hybrid PostgreSQL + Redis Shared Infrastructure
 *
  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  constructor(
    ...
    Log.info(
      this.logger,
      'ChannelProjector initialized with hybrid PostgreSQL + Redis architecture',
      {
        method: 'constructor',
  ...
  private async applyEventToDatabase(
    ...
    switch (event.type) {
      case 'NotificationSlackConfigChannelCreated.v1':
        return this.handleChannelCreated(queryRunner, event);
      case 'NotificationSlackConfigChannelUpdated.v1':
        return this.handleChannelUpdated(queryRunner, event);
      case 'NotificationSlackConfigChannelDeleted.v1':
        return this.handleChannelDeleted(queryRunner, event);
      default:
  ...
  /**
   * Generic handler for all channel events with operation-specific customization
   */
  private async handleChannelEvent(
    queryRunner: QueryRunner,
    event: ProjectionEvent,
    operation: string,
    customizer?: (params: ChannelRowParams) => void,
  ): Promise<void> {
    try {
      const params = this.extractChannelParams(event, operation);
      ...
      if (result.length > 0) {
        const mappedResult = ChannelRowMappers.mapToProjectionUpsertResult(
          result[0],
        );
        Log.debug(this.logger, `${operation} event projected successfully`, {
          method: 'handleChannelEvent',
          operation,
          eventType: event.type,
          streamId: event.streamId,
  ...
  private async handleChannelCreated(
    ...
    return this.handleChannelEvent(queryRunner, event, 'handleChannelCreated');
  }
  ...
  private async handleChannelUpdated(
    ...
    return this.handleChannelEvent(queryRunner, event, 'handleChannelUpdated');
  }
  ...
  private async handleChannelDeleted(
    ...
    return this.handleChannelEvent(
      queryRunner,
      event,
