...
export class ChannelReaderRepository implements IChannelReader {
  ...
  async findById(
    ...
    try {
      Log.debug(this.logger, 'Finding channel by code in shared store', {
        ...logContext,
        queryDetails: {
      ...
      const projection = channelStore.get(actor.tenant, code.value);
  ...
  async exists(
    ...
    try {
      ...
      const exists = channelStore.has(actor.tenant, code.value);
