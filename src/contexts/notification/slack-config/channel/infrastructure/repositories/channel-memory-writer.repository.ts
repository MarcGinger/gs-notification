...
/**
 * Channel Writer Repository - In-Memory Implementation
 *
 * Handles write operations (create, update, delete) for Channel aggregates using in-memory projector
 ...
 *
 * @domain Notification Context - Channel Writer Repository (In-Memory)
...
export class ChannelWriterRepository implements IChannelWriter {
  ...
  /**
   * Save a Channel aggregate (create or update) using In-Memory store approach
  ...
  async save(
    ...
    if (events.length === 0) {
      ...
      const receipt: SaveReceipt = {
        ...
        streamRevision: BigInt(channel.version),
    ...
    const prevVersion = channel.version - events.length;
    ...
    try {
      Log.debug(this.logger, 'Saving channel to shared store', {
        ...logContext,
        eventCount: events.length,
        prevVersion,
        queryDetails: {
          scope: 'shared_channel_store',
          method: 'channelStore.set',
        },
      });

      // Check if channel already exists for version conflict detection
      const existingProjection = channelStore.get(
        actor.tenant,
        channel.id.value,
      );
      ...
      const projection: ChannelProjection = {
        // ChannelProps fields
        code: channel.entity.code.value,
        name: channel.entity.name.value,
        workspaceCode: channel.entity.workspaceCode.value,
        isPrivate: channel.entity.isPrivate.value,
        isDm: channel.entity.isDm.value,
        topic: channel.entity.topic?.value,
        purpose: channel.entity.purpose?.value,
        subscribedEvents: channel.entity.subscribedEvents?.toArray(),
        enabled: channel.entity.enabled.value,
        // BaseProjection fields
        tenant: actor.tenant,
        version: channel.version,
        createdAt: channel.entity.createdAt.value,
        updatedAt: this.clock.now(),
        deletedAt: undefined,
        lastStreamRevision: channel.version.toString(),
      };

      // Save to in-memory store
      channelStore.set(actor.tenant, channel.id.value, projection);
  ...
  /**
   * Delete a Channel by its unique identifier using In-Memory store approach
   * @param actor - The authenticated user context
  ...
  async delete(
    ...
    try {
      ...
      if (!channelStore.has(actor.tenant || 'default-tenant', code.value)) {
      ...
      channelStore.delete(actor.tenant || 'default-tenant', code.value);
