...
/**
 * Channel Reader Repository - EventStore Implementation
 *
 * Bounded Context: Notification/Channel
 * Handles basic read operations for Channel aggregates using EventStore DB (ESDB)
 ...
 *
 * @domain Notification Context - Channel Reader Repository (EventStore)
...
export class ChannelReaderRepository implements IChannelReader {
  ...
  constructor(
    ...
    Log.info(
      this.logger,
      'ChannelReaderRepository initialized with EventStore backend',
      {
  ...
  /**
   * Generate EventStore stream name for channel aggregate
   * Uses same pattern as ChannelWriter for consistency
   */
  private generateStreamName(tenant: string, code: string): string {
    return `${ChannelProjectionKeys.getEventStoreStreamPrefix()}-${tenant}-${code}`;
  }

  /**
   * Get Channel aggregate from EventStore last event (complete aggregate state)
   */
  private async getChannelFromLastEvent(
    tenant: string,
    channelCode: string,
  ): Promise<ChannelSnapshotProps | null> {
    try {
      const streamName = this.generateStreamName(tenant, channelCode);
      ...
      if (!lastEvent.data || typeof lastEvent.data !== 'object') {
        Log.warn(
          ...
          {
            method: 'getChannelFromLastEvent',
            streamName,
      ...
      Log.error(this.logger, 'Failed to get Channel from EventStore', {
        method: 'getChannelFromLastEvent',
  ...
  async findById(
    ...
    RepositoryLoggingUtil.logAuthorizationSuccess(
      ...
      operation,
      logContext,
      { queryType: 'channel_lookup', scope: 'eventstore_stream' },
    );

    try {
      Log.debug(this.logger, 'Finding channel by code in EventStore', {
        ...logContext,
        queryDetails: {
      ...
      // Get channel from EventStore last event
      const channelSnapshot = await this.getChannelFromLastEvent(
        actor.tenant,
        code.value,
      );
  ...
  async exists(
    ...
    try {
      Log.debug(this.logger, 'Checking channel existence in EventStore', {
        ...logContext,
        queryDetails: {
          scope: 'eventstore_stream',
          method: 'getChannelFromLastEvent',
      ...
      const exists = channelSnapshot !== null;
  ...
  async getVersion(
    ...
    try {
      Log.debug(this.logger, 'Getting channel version from EventStore', {
        ...logContext,
        queryDetails: {
          scope: 'eventstore_stream',
          method: 'getChannelFromLastEvent',
      ...
      // Get channel from EventStore last event to retrieve version
      const channelSnapshot = await this.getChannelFromLastEvent(
        actor.tenant,
        code.value,
      );
      ...
      const version = channelSnapshot.version;
  ...
  async getMinimal(
    ...
    try {
      Log.debug(this.logger, 'Getting minimal channel data from EventStore', {
        ...logContext,
        queryDetails: {
          scope: 'eventstore_stream',
          method: 'getChannelFromLastEvent',
      ...
      // Get channel from EventStore last event to retrieve minimal data
      const channelSnapshot = await this.getChannelFromLastEvent(
        actor.tenant,
        code.value,
      );
