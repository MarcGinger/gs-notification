...
import { updateWorkspaceAggregateFromSnapshot } from '../../domain/factories';
...
import { IUpdateWorkspaceUseCase } from './contracts';
...
export class UpdateWorkspaceUseCase implements IUpdateWorkspaceUseCase {
  ...
  constructor(
    ...
    this.loggingConfig = {
      ...
      component: 'UpdateWorkspaceUseCase',
      domain: 'slack-config',
  ...
  async execute(params: {
    ...
    const operation = 'update_workspace';
    ...
    >({
      ...
      component: 'UpdateWorkspaceUseCase',
      operation: 'update_workspace',
      source: 'slack-config.application.update-workspace',
      command,
      logger: this.logger,
      clock: this.clock,
      repo: {
      ...
      loadExisting: async (actor, cmd) => {
        const codeR = WorkspaceId.from(cmd.id);
        if (!codeR.ok) return err(codeR.error);

        const foundR = await this.workspaceReader.findById(actor, codeR.value);
        if (!foundR.ok) {
          return err(
