...
import { WorkspaceRow, WorkspaceRowQueries } from '../schemas/workspace.schema';
 ...
 *
 * @domain Notification Context - Workspace Query Repository
...
export class WorkspaceQueryRepository implements IWorkspaceQuery {
  ...
  async findById(
    ...
    try {
      ...
      Log.debug(this.logger, 'Executing workspace findById query', {
        ...logContext,
        sql: query,
      ...
      const results = (await RepositoryExecutionUtil.executeWithTimeout(
        queryPromise,
        options?.timeout,
        'Workspace findById query timeout',
      )) as WorkspaceRow[];
      ...
      // Transform to DetailWorkspaceResponse DTO
      const detailResponse = this.toDetailResponse(workspace);
  ...
  /**
   * Find Workspace records with pagination and filtering using raw SQL.
   *
   * Leverages the proven schema and patterns from WorkspaceReaderRepository
  ...
  async findPaginated(
    ...
    try {
      ...
      const countQuery =
        WorkspaceRowQueries.COUNT_WORKSPACES_BASE +
        ' WHERE ' +
        whereConditions.join(' AND ');

      Log.debug(this.logger, 'Executing workspace count query', {
        ...logContext,
        sql: countQuery,
        parameters,
      });
      ...
      const dataQuery =
        WorkspaceRowQueries.FIND_WORKSPACES_PAGINATED_BASE +
        ' WHERE ' +
        whereConditions.join(' AND ') +
        ' ' +
        orderBy +
      ...
      Log.debug(this.logger, 'Executing paginated workspace query', {
        ...logContext,
        sql: dataQuery,
        parameters: parameters.map((p, i) => `$${i + 1}: ${String(p)}`),
        totalItems,
      ...
      const workspaces = (await RepositoryExecutionUtil.executeWithTimeout(
        queryPromise,
        options?.timeout,
        'Workspace data query timeout',
      )) as WorkspaceRow[];

      // Transform to DTOs
      const workspaceResponses = workspaces.map((workspace) =>
      ...
      RepositoryLoggingUtil.logQueryMetrics(
        ...
        logContext,
        {
          ...
          dataQuality: workspaces.length > 0 ? 'good' : 'empty',
          sampleData: {
            totalItems,
            page,
            size,
  ...
  /**
   * Transform WorkspaceRow to DetailWorkspaceResponse DTO
   * Uses the same parsing logic as toListResponse but returns DetailWorkspaceResponse
   */
  private toDetailResponse(workspace: WorkspaceRow): DetailWorkspaceResponse {
    // Parse complex JSONB objects - let undefined values be undefined (no fake defaults)

    // Create DetailWorkspaceResponse with explicit field mapping
    const response: DetailWorkspaceResponse = {
      code: workspace.code,
      name: workspace.name,
      botToken: workspace.bot_token ?? undefined,
      signingSecret: workspace.signing_secret ?? undefined,
      appId: workspace.app_id ?? undefined,
      botUserId: workspace.bot_user_id ?? undefined,
      defaultChannelId: workspace.default_channel_id ?? undefined,
  ...
  /**
   * Transform WorkspaceRow to ListWorkspaceResponse DTO
   * Uses the proven transformation patterns from WorkspaceReaderRepository
