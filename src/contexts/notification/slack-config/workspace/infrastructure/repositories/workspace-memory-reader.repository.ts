...
export class WorkspaceReaderRepository implements IWorkspaceReader {
  ...
  async findById(
    ...
    try {
      Log.debug(this.logger, 'Finding workspace by code in shared store', {
        ...logContext,
        queryDetails: {
      ...
      const projection = workspaceStore.get(actor.tenant, code.value);
  ...
  async exists(
    ...
    try {
      ...
      const exists = workspaceStore.has(actor.tenant, code.value);
