  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  loadSnapshot(snapshot: Map<string, InMemoryWorkspaceProjection>): void {
    ...
    for (const [key, projection] of snapshot) {
      ...
      workspaceStore.set(tenant, code, projection);
    }
  ...
  getProjection(key: string): InMemoryWorkspaceProjection | undefined {
    const [tenant, , code] = key.split(':');
    const projection = workspaceStore.get(tenant, code);
  ...
  private projectEvent(event: ProjectionEvent): void {
    ...
    try {
      ...
      workspaceStore.set(tenant, params.code, projection);
  ...
  private applyEventToMemory(
    ...
    const eventTs =
      hasTimestamp(event) && event.timestamp
        ...
        : this.clock.now();
    const existing = workspaceStore.get(params.tenant, params.code);
