 ...
 *
 * @domain Notification Context - Config Writer Repository
 * @layer Infrastructure
 * @pattern Repository Pattern + Interface Segregation Principle
 */
export class ConfigWriterRepository
  ...
  implements IConfigWriter
{
  ...
  /**
   * Save a Config aggregate (create or update) using EventStoreDB-first approach
  ...
  async save(
    ...
    if (events.length === 0) {
      ...
      const receipt: SaveReceipt = {
        ...
        streamRevision:
          config.version === 0 ? BigInt(-1) : BigInt(config.version - 1),
  ...
  async delete(
    ...
    const correlationId =
      ...
      CorrelationUtil.generateForOperation('config-delete');
    ...
    const eventMetadata: EventMetadata = {
      ...
      service: 'config-writer-repository',
      timestampIso: this.clock.nowIso(),
      eventVersion: '1.0',
    ...
    try {
      Log.debug(
        this.logger,
        'Appending config deletion event to EventStoreDB',
        {
          ...logContext,
          stream,
          configWorkspaceCode: workspaceCode.value,
