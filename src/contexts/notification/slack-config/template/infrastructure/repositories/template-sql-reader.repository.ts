...
/**
 * Template Reader Repository - Interface Segregation Principle Implementation
 *
 * Bounded Context: Notification/Template
 * Handles basic read operations for Template aggregates with comprehensive
 ...
 *
 * @domain Notification Context - Template Reader Repository
...
export class TemplateReaderRepository implements ITemplateReader {
  ...
  async findById(
    ...
    RepositoryLoggingUtil.logAuthorizationSuccess(
      ...
      operation,
      logContext,
      { queryType: 'template_lookup', scope: 'template_snapshot' },
    );

    try {
      Log.debug(this.logger, 'Finding template by code', {
        ...logContext,
        queryDetails: { scope: 'template_snapshot', orderBy: 'code' },
      });

      // Query the template projection table with all JSONB fields using centralized query
      ...
      const variables = row.variables
        ? safeParseJSONArray(row.variables, 'variables', isString)
        : undefined;
      ...
      Log.debug(this.logger, 'Parsing Template JSON fields', {
        ...logContext,
        jsonFields: {
      ...
      // Create TemplateSnapshot mapping exactly what's persisted
  ...
  async exists(
    ...
    RepositoryLoggingUtil.logAuthorizationSuccess(
      ...
      operation,
      logContext,
      { queryType: 'existence_check', scope: 'template_validation' },
    );

    try {
      Log.debug(this.logger, 'Checking template existence', {
        ...logContext,
        queryDetails: { scope: 'existence_check', optimized: true },
      });

      // Optimized existence check query using centralized query
      const query = TemplateRowQueries.CHECK_TEMPLATE_EXISTS;
      ...
      const result: TemplateExistsRow[] =
        (await RepositoryExecutionUtil.executeWithTimeout(
          queryPromise,
          options?.timeout,
          'Template existence check timeout',
        )) as TemplateExistsRow[];
  ...
  async getVersion(
    ...
    try {
      ...
      const result: TemplateVersionRow[] =
        (await RepositoryExecutionUtil.executeWithTimeout(
          queryPromise,
          options?.timeout,
          'Template version lookup timeout',
  ...
  async getMinimal(
    ...
    try {
      ...
      const result: TemplateMinimalRow[] =
        (await RepositoryExecutionUtil.executeWithTimeout(
          queryPromise,
          options?.timeout,
          'Template minimal lookup timeout',
        )) as TemplateMinimalRow[];
