...
export class TemplateReaderRepository implements ITemplateReader {
  ...
  async findById(
    ...
    try {
      Log.debug(this.logger, 'Finding template by code in shared store', {
        ...logContext,
        queryDetails: {
      ...
      const projection = templateStore.get(actor.tenant, code.value);
  ...
  async exists(
    ...
    try {
      ...
      const exists = templateStore.has(actor.tenant, code.value);
