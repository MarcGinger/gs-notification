  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  loadSnapshot(snapshot: Map<string, InMemoryTemplateProjection>): void {
    ...
    for (const [key, projection] of snapshot) {
      const [tenantId, , code] = key.split(':');
      templateStore.set(tenantId, code, projection);
    }
  ...
  getProjection(key: string): InMemoryTemplateProjection | undefined {
    const [tenantId, , code] = key.split(':');
    const projection = templateStore.get(tenantId, code);
  ...
  private applyEventToMemory(
    ...
    const eventTs =
      hasTimestamp(event) && event.timestamp
        ...
        : this.clock.now();
    const existing = templateStore.get(params.tenantId, params.code);
   ...
   */
  private buildProjectionKey(tenantId: string, code: string): string {
    return `${tenantId}:template:${code}`;
  }
  ...
  private extractTemplateParams(
    ...
    try {
      ...
      // Use TemplateFieldValidatorUtil to create validated initiation fee snapshot
      const templateSnapshot =
        TemplateFieldValidatorUtil.createTemplateSnapshotFromEventData(
          eventData,
        );
