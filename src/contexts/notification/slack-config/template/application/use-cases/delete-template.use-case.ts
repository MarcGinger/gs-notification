...
import { deleteTemplateAggregateFromSnapshot } from '../../domain/factories';
import { TemplateErrors } from '../../domain/errors';
import { IDeleteTemplateUseCase } from './contracts';
...
type TemplateSnapshot = Parameters<
  typeof deleteTemplateAggregateFromSnapshot
>[0];

/**
 * Delete Template Use Case
 *
 * Handles the secure deletion of existing templates with comprehensive
 ...
 *
 * @domain Notification Context - Template Use Cases
...
export class DeleteTemplateUseCase implements IDeleteTemplateUseCase {
  ...
  constructor(
    ...
    this.loggingConfig = {
      ...
      component: 'DeleteTemplateUseCase',
      domain: 'slack-config',
      entityType: 'template',
    };
  ...
  /**
   * Execute the delete template use case with enhanced security and compliance
   * @param command - The delete command containing template identifier and context
  ...
  async execute(params: {
    ...
    try {
      ...
      >({
        ...
        source: 'slack-config.application.delete-template',
        command,
        logger: this.logger,
        clock: this.clock,
        repo: {
          save: async (
            ...
            const saveResult = await this.templateWriter.save(
              actor,
              template,
              expectedVersion,
            );
            ...
            const codeResult = TemplateCode.from(rawProps.code);
            if (!codeResult.ok) return err(codeResult.error);

            const deleteResult = await this.templateWriter.delete(
              actor,
              codeResult.value,
            );
        ...
        // Load existing snapshot or return TEMPLATE_NOT_FOUND with context
        loadExisting: async (actor, cmd) => {
          const codeR = TemplateCode.from(rawProps.code);
        ...
        runDomain: ({ existing, metadata, clock }) => {
          ...
          return deleteTemplateAggregateFromSnapshot(
            existing!,
            enhancedMetadata,
            clock,
          );
      ...
      const domainError = withContext(
        TemplateErrors.CANNOT_DELETE_TEMPLATE_WITH_DEPENDENCIES,
        {
