...
export class TemplateAggregate extends AggregateRootBase {
  ...
  protected when(event: DomainEvent): void {
    switch (event.type) {
      ...
      case 'TemplateUpdated': {
        ...
        const entityResult = TemplateEntity.fromSnapshot(d.after);
        if (entityResult.ok) {
          this._entity = entityResult.value;
        }
        break;
  ...
  private buildChangeSet(
    ...
    const cs: TemplateChangeSet = {};

    // Simple field comparisons
    compareField(before.workspaceId, after.workspaceId, 'workspaceId', cs);
    compareField(before.name, after.name, 'name', cs);
    compareField(before.description, after.description, 'description', cs);
    compareComplexField(
      before.contentBlocks,
      after.contentBlocks,
      'contentBlocks',
      cs,
    );
    compareComplexField(before.variables, after.variables, 'variables', cs);
    compareComplexField(
      before.samplePayload,
      after.samplePayload,
      'samplePayload',
      cs,
    );
  ...
  /**
   * Helper method to publish TemplateUpdatedEvent with consistent structure
   *
  ...
  private publishUpdateEvent(
    ...
    // Create typed TemplateUpdatedEvent with full payload
