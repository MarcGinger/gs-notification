 ...
 *
 * @domain NotificationSlack Context - AppConfig Writer Repository
 * @layer Infrastructure
 * @pattern Repository Pattern + Interface Segregation Principle
 */
export class AppConfigWriterRepository
  extends BaseWriterRepository
  implements IAppConfigWriter
{
  ...
  /**
   * Save a AppConfig aggregate (create or update) using EventStoreDB-first approach
  ...
  async save(
    ...
    if (events.length === 0) {
      ...
      const receipt: SaveReceipt = {
        ...
        streamRevision:
          appConfig.version === 0 ? BigInt(-1) : BigInt(appConfig.version - 1),
    ...
    const prevVersion = appConfig.version - eventsToStore.length;
  ...
  async delete(
    ...
    const eventMetadata: EventMetadata = {
      ...
      service: 'app-config-writer-repository',
      timestampIso: this.clock.nowIso(),
      eventVersion: '1.0',
    ...
    try {
      Log.debug(
        this.logger,
        'Appending app-config deletion event to EventStoreDB',
        {
          ...logContext,
          stream,
          appConfigId: id.toString(),
