...
/**
 * notification-slack slack-config application/service Documentation
 * This module handles the Swagger documentation for notification-slacks
 *
 */
...
### application: slack-config
  ...
  | Modules |
  |---------------|
  | [***workspace  →***](/api/docs/notification-slack/slack-config/workspace) |
  | [***channel  →***](/api/docs/notification-slack/slack-config/channel) |
  | [***template  →***](/api/docs/notification-slack/slack-config/template) |
  | [***app-config  →***](/api/docs/notification-slack/slack-config/app-config) |
        `,
      )
      .setVersion('1.0.0')
      .addTag(
        'SlackConfig',
        `Configuration for notification-slack slack-config application/service`,
      );
    ...
    SwaggerModule.setup(
      'api/docs/notification-slack/slack-config',
      app,
      document,
    );
  }
  ...
  /**
   * Setup all notification_slack-related documentation endpoints
   * This method centralizes the setup of all notification_slack module documentation
   */
  static setupAll(
    app: INestApplication,
    port: string | number,
  ): Record<string, string> {
    // Setup main notification_slack documentation
    ...
    return {
      notification_slack: SlackConfigDocumentation.getEndpoint(port),
    };
  }

  static getEndpoint(port: string | number): string {
    return `${SwaggerConfigUtil.getServerUrl(port)}/api/docs/notification-slack/slack-config`;
  }
}
