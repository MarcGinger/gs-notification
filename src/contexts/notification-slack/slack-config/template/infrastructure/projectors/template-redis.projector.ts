...
// Template Projector - ESDB to Redis Projection
...
// ✅ Import templateion-ready shared utilities
...
import { NotificationSlackSlackProjectorConfig } from '../../../projector.config';
...
/**
 * Template row parameters for Redis projection operations
 *
 * Extends DetailTemplateResponse DTO with projection-specific fields needed for event sourcing.
 *
 * Key Additions to DetailTemplateResponse:
...
/**
 * Template Projector using Redis-based Shared Projection Infrastructure
 *
 ...
 *
 * Redis Data Structure (managed by TemplateProjectionKeys domain value object):
 * - Template Hash: `template:projection:{tenantId}:code` - stores all template fields
 * - Tenant Index: `template:index:by_tenant:{tenantId}` - sorted set by updated timestamp
 *
 * Key Features:
  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  // ✅ Templateion-ready shared utilities
  ...
  constructor(
    ...
    // ✅ Initialize shared utilities for templateion-ready operations
    this.cacheOptimization = new CacheOptimizationUtils();
    this.pipelineBuilder = new RedisPipelineBuilder();

    Log.info(
      this.logger,
      'TemplateProjector initialized with templateion-ready shared utilities',
      {
        method: 'constructor',
  ...
  /**
   * Start the projector using CatchUpRunner with templateion-ready utilities
   */
  onModuleInit(): void {
  ...
  /**
   * Project individual event using templateion-ready EventPipelineProcessor
  ...
  private async projectEvent(
    ...
    try {
      ...
      const indexKey = TemplateProjectionKeys.getRedisTemplateIndexKey(tenant);
  ...
  private generateClusterSafeKeys(params: TemplateRowParams): {
    ...
    const entityKey = 'template:{' + params.tenantId + '}:' + params.code;
    const indexKey = 'template:index:{' + params.tenantId + '}';
