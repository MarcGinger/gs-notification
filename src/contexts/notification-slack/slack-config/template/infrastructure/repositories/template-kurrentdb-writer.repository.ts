 ...
 *
 * @domain NotificationSlack Context - Template Writer Repository
 * @layer Infrastructure
 * @pattern Repository Pattern + Interface Segregation Principle
 */
export class TemplateWriterRepository
  ...
  implements ITemplateWriter
{
  ...
  constructor(
    ...
    super();
    this.loggingConfig = {
      serviceName: 'NotificationSlackSlackConfigService',
  ...
  /**
   * Save a Template aggregate (create or update) using EventStoreDB-first approach
  ...
  async save(
    ...
    if (events.length === 0) {
      ...
      const receipt: SaveReceipt = {
        ...
        streamRevision:
          template.version === 0 ? BigInt(-1) : BigInt(template.version - 1),
    ...
    const prevVersion = template.version - eventsToStore.length;
  ...
  /**
   * Delete a Template by its unique identifier using EventStoreDB-first approach
   * @param actor - The authenticated user context
  ...
  async delete(
    ...
    const eventMetadata: EventMetadata = {
      ...
      service: 'template-writer-repository',
      timestampIso: this.clock.nowIso(),
      eventVersion: '1.0',
      schemaVersion: '2023.1',
      source: 'notification-slack.domain.template',
    };

    // Create proper TemplateDeletedEvent
    const deletedEvent = TemplateDeletedEvent.create(
      {
        code: code.value,
    ...
    try {
      Log.debug(
        this.logger,
        'Appending template deletion event to EventStoreDB',
        {
          ...logContext,
          stream,
          templateCode: code.value,
      ...
      RepositoryLoggingUtil.logQueryMetrics(
        ...
        logContext,
        {
          ...
          sampleData: {
            ...
            templateCode: code.value,
            streamRevision: appendResult?.nextExpectedRevision?.toString(),
            eventType: 'TemplateDeletedEvent',
          },
        },
      );
