...
/**
 * AttributeRule State Infrastructure Mappers
 *
...
import { AttributeRuleDomainState } from '../../domain/state/attribute-rule.state';
 ...
 */
export class AttributeRuleStateMapper {
  /**
  ...
  static toDomain(
  ...
  ): Result<AttributeRuleDomainState, DomainError> {
    ...
    const code = validateField('code', AttributeRuleCode.from(snapshot.code));
    const name = validateField('name', AttributeRuleName.from(snapshot.name));
    const description = snapshot.description
      ? validateField(
          'description',
          AttributeRuleDescription.from(snapshot.description),
        )
      : undefined;
    const enabled = snapshot.enabled
      ? validateField('enabled', AttributeRuleEnabled.from(snapshot.enabled))
      : undefined;
    const attributes = validateField(
      'attributes',
      AttributeRuleRuleConfiguration.from(snapshot.attributes),
    );
    const version = validateField(
      'version',
      AttributeRuleVersion.from(snapshot.version),
    );
    const createdAt = validateField(
      'createdAt',
      AttributeRuleCreatedAt.from(snapshot.createdAt),
    );
    const updatedAt = validateField(
      'updatedAt',
      AttributeRuleUpdatedAt.from(snapshot.updatedAt),
    );

    // If any validations failed, return aggregated error
    if (errors.length > 0) {
      return err({
        code: 'ATTRIBUTE_RULE.MAPPER_VALIDATION_FAILED',
    ...
    const domainState: AttributeRuleDomainState = {
      code: code!,
      name: name!,
  ...
  static toSnapshot(
    ...
    return {
      ...
      attributes: ruleConfigurationToSnapshotProps(domainState.attributes), // Complex object - stored as-is for now
