...
import {
  ...
  ApiAttributeRuleListMeta,
  ApiAttributeRuleName,
} from './decorators';
import { ListAttributeRuleResponse } from './';
/**
 * AttributeRule list request DTO with pagination and filtering
 * Combines standardized pagination with attribute-rule-specific filters
 ...
 */
export class ListAttributeRuleFilterRequest extends BaseListFilterRequest {
  ...
  /**
   * Define available sort fields for attributeRules
   * This method ensures type-safe sorting validation
   *
   * Available sort fields:
   * - code: AttributeRule code (sortable, default: asc)
   * - name: AttributeRule name (sortable, default: asc)
   *
  ...
  getSortFieldConfig(): SortFieldConfig {
    return {
      code: {
        description: 'AttributeRule code',
        sortable: true,
        defaultDirection: 'asc' as const,
      },
      name: {
        description: 'AttributeRule name',
        sortable: true,
        defaultDirection: 'asc' as const,
      },
    };
...
/**
 * AttributeRule page response DTO with metadata for pagination
 * Extends the generic PagedResponse pattern while maintaining API compatibility
 */
export class AttributeRulePageResponse {
  @ApiListOf(ListAttributeRuleResponse)
  readonly data: ListAttributeRuleResponse[];

  @ApiAttributeRuleListMeta()
  readonly meta: PaginationMetaResponse;

  constructor(data: ListAttributeRuleResponse[], meta: PaginationMetaResponse) {
    this.data = data;
    this.meta = meta;
  }
   ...
   */
  static create(
    data: ListAttributeRuleResponse[],
    meta: PaginationMetaResponse,
  ): AttributeRulePageResponse {
    return new AttributeRulePageResponse(data, meta);
  }
