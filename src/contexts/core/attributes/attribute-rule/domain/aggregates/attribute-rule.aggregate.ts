...
import { AttributeRuleSnapshotProps, RuleProps } from '../props';
import { ValidatedAttributeRuleUpdateFields } from '../types';
import { extractRuleConfigurationData } from '../utilities';
import {
  AttributeRuleCode,
  createAttributeRuleCreatedAt,
  createAttributeRuleUpdatedAt,
  createdAtNow,
  updatedAtNow,
  validateMonotonicTime,
  bumpVersion,
  initialVersion,
  createAttributeRuleVersion,
...
import { AttributeRuleErrors } from '../errors';
import { AttributeRuleDomainState } from '../state';

/**
 * Domain Aggregate Root: AttributeRule
 *
 * Represents the AttributeRule aggregate root in the core domain.
 * Encapsulates attributeRule business logic, state management, and domain invariants.
 *
 ...
 *
 * @domain Core Context - AttributeRule Aggregate
 * @layer Domain Aggregates
 */

/**
 * AttributeRule Aggregate Root
 *
 * Central aggregate root that coordinates attributeRule business operations.
 * Manages consistency across the aggregate boundary and publishes domain events.
 */
export class AttributeRuleAggregate extends AggregateRootBase {
  private _entity: AttributeRuleEntity;
  private readonly clock: Clock;
  private readonly eventMetadata: EventMetadata;

  private constructor(
    entity: AttributeRuleEntity,
    clock: Clock,
  ...
  /**
   * Factory method to create a new AttributeRule aggregate
   *
   * @param props - AttributeRule entity properties (should include createdAt, version)
   * @param clock - Clock instance for time operations
   * @param eventMetadata - Event metadata for domain events
   * @returns Result containing AttributeRuleAggregate or DomainError
   */
  public static create(
    ...
    const createdAtResult = props.createdAt
      ? createAttributeRuleCreatedAt(props.createdAt.value) // Extract Date from existing VO
      : createdAtNow(clock);
    ...
    const updatedAtResult = props.updatedAt
      ? createAttributeRuleUpdatedAt(props.updatedAt.value) // Extract Date from existing VO
      : updatedAtNow(clock);
    ...
    const versionResult =
      props.version !== undefined
        ? AttributeRuleVersion.create(props.version.value) // Extract number from existing VO
        : initialVersion();
    ...
    const entityResult = AttributeRuleEntity.create(entityProps);
    if (!entityResult.ok) {
      return err(entityResult.error);
    }

    // Create the aggregate
    const aggregate = new AttributeRuleAggregate(
      entityResult.value,
      clock,
      eventMetadata,
    );

    // Create typed AttributeRuleCreatedEvent with only business data
    const createdEvent = AttributeRuleCreatedEvent.create({
      ...
      attributes: extractRuleConfigurationData(entityProps.attributes),
    });
   ...
   * @param eventMetadata - Event metadata for domain events
   * @returns AttributeRuleAggregate
   */
  public static reconstitute(
    ...
    const aggregate = new AttributeRuleAggregate(entity, clock, eventMetadata);
  ...
  protected when(event: DomainEvent): void {
    switch (event.type) {
      ...
      case 'CoreAttributesAttributeRuleUpdated.v1': {
        ...
        const entityResult = AttributeRuleEntity.fromSnapshot({
          code: d.code,
          name: d.name,
  ...
  protected applySnapshot(
    ...
    const entityResult = AttributeRuleEntity.fromSnapshot(snapshot);

    if (!entityResult.ok) {
      return err({
        code: 'ATTRIBUTE_RULE.SNAPSHOT_APPLICATION_FAILED',
   ...
   */
  public createSnapshot(): AttributeRuleSnapshotProps {
   ...
   */
  public toDomainState(): AttributeRuleDomainState {
   ...
   */
  public get id(): AttributeRuleCode {
   ...
   */
  public get entity(): AttributeRuleEntity {
    return this._entity;
  }
  ...
  private detectChanges(
    validatedFields: ValidatedAttributeRuleUpdateFields,
  ): boolean {
  ...
  public updateBatch(
    ...
    if (
      ...
      return err({
        ...AttributeRuleErrors.CONCURRENCY_CONFLICT,
        context: {
    ...
    const currentVersionResult = createAttributeRuleVersion(before.version);
    ...
    const updatedEvent = AttributeRuleUpdatedEvent.create({
      ...
      attributes: extractRuleConfigurationData(this._entity.attributes),
    });
  ...
  /**
   * Delete the attributeRule entity (business operation with event publishing)
   *
   * @returns Result indicating success or failure
   */
   ...
   */
  public sameAs(other: AttributeRuleAggregate): boolean {
