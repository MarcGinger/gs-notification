...
import {
  ...
  UpsertAttributeRuleSetProps,
} from '../../domain/props';
import { AttributeRuleSetCode } from '../../domain/value-objects';
import {
  createAttributeRuleSetAggregateFromProps,
  updateAttributeRuleSetAggregateFromSnapshot,
...
import { IUpsertAttributeRuleSetUseCase } from './contracts';

import { AttributeRuleSetAuthorizationAdapter } from '../services';
import { DetailAttributeRuleSetResponse } from '../dtos';
import { AttributeRuleSetDtoAssembler } from '../assemblers';

type AttributeRuleSetSnapshot = Parameters<
  typeof updateAttributeRuleSetAggregateFromSnapshot
>[0];

@Injectable()
export class UpsertAttributeRuleSetUseCase
  implements IUpsertAttributeRuleSetUseCase
{
  ...
  constructor(
    ...
    this.loggingConfig = {
      ...
      component: 'UpsertAttributeRuleSetUseCase',
      domain: 'attributes',
  ...
  async execute(params: {
    ...
    const operation = 'upsert_attribute_rule_set';
    ...
    const safeLogContext = UseCaseLoggingUtil.createLogContext(
      ...
      {
        attributeRuleSetCode: command.code,
    ...
    // Note: UpsertAttributeRuleSetProps approach provides cleaner validation semantics
    ...
    const codeR = AttributeRuleSetCode.create(command.code);
    ...
    const existsR = await this.attributeRuleSetReader.exists(
      actor,
      codeR.value,
    );
    ...
    >({
      ...
      component: 'UpsertAttributeRuleSetUseCase',
      operation: isCreate
        ? 'create_attribute_rule_set'
        : 'update_attribute_rule_set',
      source: 'attribute-rule-set-config.application.upsert-attribute-rule-set',
      command,
      logger: this.logger,
      clock: this.clock,
      ...
      propsMissingError: AttributeRuleSetErrors.INVALID_ATTRIBUTE_RULE_SET_DATA,
      ...
      loadExisting: async () => {
        if (!isCreate) {
          ...
          if (Option.isNone(foundR.value)) {
            return ok(undefined as unknown as AttributeRuleSetSnapshot);
          }
        ...
        return Promise.resolve(
          ok(undefined as unknown as AttributeRuleSetSnapshot),
        );
      },
      ...
      runDomain: ({ existing, metadata, clock }) => {
        if (existing) {
          ...
          return updateAttributeRuleSetAggregateFromSnapshot(
            existing,
            rawProps || {},
            metadata,
            clock,
        ...
        const createProps = upsertProps as CreateAttributeRuleSetProps;
        return createAttributeRuleSetAggregateFromProps(
          createProps,
          metadata,
          clock,
        );
    ...
    const dto = AttributeRuleSetDtoAssembler.toDetailResponse(domainState);
