...
// AttributeRuleSet Projector - ESDB to In-Memory Projection
...
const AttributeRuleSetProjectorErrors = createProjectorErrorCatalog(
  'ATTRIBUTE_RULE_SET_PROJECTOR_INMEMORY',
  CommonProjectorErrorDefinitions,
);

/**
 * Extracted attributeRuleSet parameters from event data
 */
interface ExtractedAttributeRuleSetParams
...
/**
 * Extended attributeRuleSet projection with projector-specific metadata
 */
interface InMemoryAttributeRuleSetProjection
  extends AttributeRuleSetProjection {
...
/**
 * AttributeRuleSet Projector using In-Memory Architecture
 *
 * Perfect for:
  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  private shadowCallback?: (
    key: string,
    primaryResult: InMemoryAttributeRuleSetProjection,
    shadowResult?: InMemoryAttributeRuleSetProjection,
  ...
  constructor(
    ...
    super(
      'attribute-rule-set-projector-inmemory',
      'attribute-rule-set-projection-inmemory',
      baseLogger,
      inMemoryCheckpointStore,
    );
    ...
    Log.info(
      this.logger,
      'AttributeRuleSetProjectorInMemory initialized using shared attributeRuleSet store',
      {
        method: 'constructor',
  ...
  enableShadowMode(
    callback: (
      key: string,
      inMemoryProjection: InMemoryAttributeRuleSetProjection,
   ...
   */
  loadSnapshot(
    snapshot: Map<string, InMemoryAttributeRuleSetProjection>,
  ): void {
    attributeRuleSetStore.clear();

    for (const [key, projection] of snapshot) {
      const [tenant, , code] = key.split(':');
      attributeRuleSetStore.set(tenant, code, projection);
    }
   ...
   */
  createSnapshot(): Map<string, InMemoryAttributeRuleSetProjection> {
    const snapshot = new Map<string, InMemoryAttributeRuleSetProjection>();

    for (const [key, projection] of attributeRuleSetStore.getAll()) {
      // Add projector-specific metadata to base projection
      const extendedProjection: InMemoryAttributeRuleSetProjection = {
        ...projection,
   ...
   */
  getProjection(key: string): InMemoryAttributeRuleSetProjection | undefined {
    const [tenant, , code] = key.split(':');
    const projection = attributeRuleSetStore.get(tenant, code);
   ...
   */
  getAllProjections(): Array<[string, InMemoryAttributeRuleSetProjection]> {
    return attributeRuleSetStore.getAll().map(([key, projection]) => [
      key,
      {
        ...projection,
        projectionTimestamp: new Date(),
  ...
  onModuleInit(): void {
    Log.info(
      this.logger,
      'Starting AttributeRuleSet Projector (In-Memory) with CatchUpRunner',
      {
        method: 'onModuleInit',
        subscriptionGroup: this.subscriptionGroup,
        architecture: 'in-memory',
        maxCacheSize: 5000, // From attributeRuleSet store
      },
    );
    ...
    try {
      ...
      Log.info(
        this.logger,
        'AttributeRuleSet Projector (In-Memory) started successfully',
        {
          method: 'onModuleInit',
          status: 'running',
      ...
      Log.error(
        this.logger,
        'Failed to start AttributeRuleSet Projector (In-Memory)',
        {
          method: 'onModuleInit',
          error: e.message,
          stack: e.stack,
  ...
  onModuleDestroy(): void {
    Log.info(this.logger, 'Stopping AttributeRuleSet Projector (In-Memory)', {
    ...
    try {
      ...
      Log.info(
        this.logger,
        'AttributeRuleSet Projector (In-Memory) stopped successfully',
        {
          method: 'onModuleDestroy',
          status: 'stopped',
          totalEventsProcessed: this.metrics.getMetrics().totalEvents,
          finalCacheSize: attributeRuleSetStore.size(),
        },
      );
    } catch (error) {
      const e = error as Error;
      Log.error(
        this.logger,
        'Error stopping AttributeRuleSet Projector (In-Memory)',
        {
          method: 'onModuleDestroy',
          error: e.message,
          stack: e.stack,
  ...
  private projectEvent(event: ProjectionEvent): void {
    ...
    try {
      ...
      // Store in shared attributeRuleSet store
      attributeRuleSetStore.set(tenant, params.code, projection);
      ...
      Log.debug(this.logger, 'Event projected to in-memory', {
        ...
        latencyMs,
        cacheSize: attributeRuleSetStore.size(),
        outcome: 'applied',
      });
  ...
  private applyEventToMemory(
    event: ProjectionEvent,
    params: ExtractedAttributeRuleSetParams,
  ): InMemoryAttributeRuleSetProjection {
    ...
    const eventTs =
      hasTimestamp(event) && event.timestamp
        ...
        : this.clock.now();
    const existing = attributeRuleSetStore.get(params.tenant, params.code);
    ...
    switch (event.type) {
      case 'CoreAttributesAttributeRuleSetCreated.v1':
        return this.handleAttributeRuleSetCreated(params, event, eventTs);
      case 'CoreAttributesAttributeRuleSetUpdated.v1':
        return this.handleAttributeRuleSetUpdated(params, event, eventTs);
      case 'CoreAttributesAttributeRuleSetDeleted.v1':
        return this.handleAttributeRuleSetDeleted(params, event, eventTs);
      default:
   ...
   */
  private handleAttributeRuleSetCreated(
    params: ExtractedAttributeRuleSetParams,
    event: ProjectionEvent,
    timestamp: Date,
  ): InMemoryAttributeRuleSetProjection {
   ...
   */
  private handleAttributeRuleSetUpdated(
    params: ExtractedAttributeRuleSetParams,
    event: ProjectionEvent,
    timestamp: Date,
  ): InMemoryAttributeRuleSetProjection {
   ...
   */
  private handleAttributeRuleSetDeleted(
    params: ExtractedAttributeRuleSetParams,
    event: ProjectionEvent,
    timestamp: Date,
  ): InMemoryAttributeRuleSetProjection {
  ...
  private buildProjectionKey(tenant: string, code: string): string {
    return `${tenant}:attribute-rule-set:${code}`;
  }
   ...
   *
   * Uses AttributeRuleSetFieldValidatorUtil to create validated DetailAttributeRuleSetResponse for consistent
  ...
  private extractAttributeRuleSetParams(
    event: ProjectionEvent,
    operation: string,
  ): ExtractedAttributeRuleSetParams {
    try {
      ...
      >(attributeRuleSetSnapshot.attributes, 'attributes');
  ...
  private updateMetrics(): void {
    this.metrics.updateEntityCount(attributeRuleSetStore.size());
  ...
  private estimateMemoryUsage(): number {
    ...
    return attributeRuleSetStore.size() * 1024;
  }
}
