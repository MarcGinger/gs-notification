...
// AttributeRuleSet Projector - ESDB to Redis Projection
...
// ✅ Import attribute-rule-setion-ready shared utilities
...
/**
 * AttributeRuleSet projection parameters for unified processing
 */
interface AttributeRuleSetProjectionParams extends UnifiedProjectionParams {
...
/**
 * AttributeRuleSet Projector using Redis-based Shared Projection Infrastructure
 *
 ...
 *
 * Redis Data Structure (managed by AttributeRuleSetProjectionKeys domain value object):
 * - AttributeRuleSet Hash: `attribute-rule-set:projection:{tenant}:code` - stores all attribute-rule-set fields
 * - Tenant Index: `attribute-rule-set:index:by_tenant:{tenant}` - sorted set by updated timestamp
 *
 * Key Features:
  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  private static readonly UNIFIED_CONFIG: UnifiedProjectionConfig = {
    ...
    getRedisEntityKey: (tenant: string, id: string) =>
      AttributeRuleSetProjectionKeys.getRedisAttributeRuleSetKey(tenant, id),
    getRedisIndexKey: (tenant: string) =>
      AttributeRuleSetProjectionKeys.getRedisAttributeRuleSetIndexKey(tenant),
    getEntityId: (params) => (params as AttributeRuleSetProjectionParams).code,
  };
  ...
  constructor(
    ...
    super(
      AttributeRuleSetProjectionKeys.PROJECTOR_NAME,
      AttributeRuleSetProjectionKeys.SUBSCRIPTION_GROUP,
      baseLogger,
      checkpointStore,
    );
    ...
    this.projectorUtils = new UnifiedProjectorUtils(
      AttributeRuleSetProjector.UNIFIED_CONFIG,
      this.redis,
    ...
    Log.info(
      this.logger,
      'AttributeRuleSetProjector initialized with production-ready utilities',
      {
        method: 'constructor',
  ...
  /**
   * Start the projector using CatchUpRunner with attribute-rule-setion-ready utilities
   */
  onModuleInit(): void {
    Log.info(
      this.logger,
      'Starting AttributeRuleSet Projector with production-ready features',
      {
        method: 'onModuleInit',
        subscriptionGroup: this.subscriptionGroup,
      },
  ...
  /**
   * Extract attribute-rule-set parameters with enhanced error context
   *
  ...
  private extractAttributeRuleSetParams(
    event: ProjectionEvent,
    operation: string,
  ): AttributeRuleSetProjectionParams {
    try {
      ...
      // Use existing AttributeRuleSetFieldValidatorUtil for validation
      const attributeRuleSetData =
