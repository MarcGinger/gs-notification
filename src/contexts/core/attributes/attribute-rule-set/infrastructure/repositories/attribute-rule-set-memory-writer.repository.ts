...
/**
 * AttributeRuleSet Writer Repository - In-Memory Implementation
 *
 * Handles write operations (create, update, delete) for AttributeRuleSet aggregates using in-memory projector
 ...
 *
 * @domain Core Context - AttributeRuleSet Writer Repository (In-Memory)
  ...
  implements IAttributeRuleSetWriter
{
  ...
  async save(
    ...
    if (events.length === 0) {
      ...
      const receipt: SaveReceipt = {
        ...
        streamRevision: BigInt(attributeRuleSet.version),
    ...
    const prevVersion = attributeRuleSet.version - events.length;
    ...
    try {
      Log.debug(this.logger, 'Saving attribute-rule-set to shared store', {
        ...logContext,
        eventCount: events.length,
        prevVersion,
        queryDetails: {
          scope: 'shared_attribute-rule-set_store',
          method: 'attributeRuleSetStore.set',
        },
      });

      // Check if attribute-rule-set already exists for version conflict detection
      const existingProjection = attributeRuleSetStore.get(
        actor.tenant,
      ...
      const projection: AttributeRuleSetProjection = {
        // AttributeRuleSetProps fields
        code: attributeRuleSet.entity.code.value,
        name: attributeRuleSet.entity.name.value,
        description: attributeRuleSet.entity.description?.value,
        enabled: attributeRuleSet.entity.enabled?.value,
        attributes: attributeRuleSet.entity.attributes
        ...
        tenant: actor.tenant,
        version: attributeRuleSet.version,
        createdAt: attributeRuleSet.entity.createdAt.value,
        updatedAt: this.clock.now(),
        deletedAt: undefined,
        lastStreamRevision: attributeRuleSet.version.toString(),
      };

      // Save to in-memory store
      attributeRuleSetStore.set(
        actor.tenant,
        attributeRuleSet.id.value,
        projection,
      );
      ...
      RepositoryLoggingUtil.logQueryMetrics(
        ...
        logContext,
        {
          ...
          sampleData: {
            ...
            code: attributeRuleSet.id.value,
          },
        },
      );
  ...
  async delete(
    ...
    try {
      // Check if attribute-rule-set exists
      if (
        !attributeRuleSetStore.has(actor.tenant || 'default-tenant', code.value)
      ) {
