...
import { ILookupWriter } from '../../application/ports';
import { lookupStore, LookupProjection } from '../stores/lookup.store';

/**
 * Lookup Writer Repository - In-Memory Implementation
 *
 * Handles write operations (create, update, delete) for Lookup aggregates using in-memory projector
 ...
 *
 * @domain Core Context - Lookup Writer Repository (In-Memory)
 * @layer Infrastructure
 * @pattern Repository Pattern + Interface Segregation Principle + In-Memory Store
 */
@Injectable()
export class LookupWriterRepository implements ILookupWriter {
  ...
  constructor(
    ...
    this.loggingConfig = {
      ...
      component: 'LookupWriterRepository',
    };
  ...
  /**
   * Save a Lookup aggregate (create or update) using In-Memory store approach
  ...
  async save(
    ...
    const correlationId = CorrelationUtil.generateForOperation('lookup-save');

    const logContext = this.createLogContext(operation, correlationId, actor, {
      riskLevel,
      aggregateId: lookup.id.value,
      currentVersion: lookup.version,
      eventCount: lookup.uncommittedEvents?.length ?? 0,
    ...
    const events = lookup.uncommittedEvents ?? [];
    ...
    if (events.length === 0) {
      ...
      const receipt: SaveReceipt = {
        stream: `lookup-${lookup.id.value}`,
        aggregateId: lookup.id.value,
        tenant: actor.tenant,
        eventCount: 0,
        newVersion: lookup.version,
        streamRevision: BigInt(lookup.version),
    ...
    const prevVersion = lookup.version - events.length;
    ...
    try {
      Log.debug(this.logger, 'Saving lookup to shared store', {
        ...logContext,
        eventCount: events.length,
        prevVersion,
        queryDetails: {
          scope: 'shared_lookup_store',
          method: 'lookupStore.set',
        },
      });

      // Check if lookup already exists for version conflict detection
      const existingProjection = lookupStore.get(actor.tenant, lookup.id.value);
      ...
      const projection: LookupProjection = {
        // LookupProps fields
        lookupType: lookup.entity.lookupType.value,
        code: lookup.entity.code.value,
        name: lookup.entity.name.value,
        description: lookup.entity.description?.value,
        enabled: lookup.entity.enabled?.value,
        attributes: lookup.entity.attributes?.value,
        // BaseProjection fields
        tenant: actor.tenant,
        version: lookup.version,
        createdAt: lookup.entity.createdAt.value,
        updatedAt: this.clock.now(),
        deletedAt: undefined,
        lastStreamRevision: lookup.version.toString(),
      };

      // Save to in-memory store
      lookupStore.set(actor.tenant, lookup.id.value, projection);

      // Mark committed after successful store save
      lookup.markEventsAsCommitted();
  ...
  /**
   * Delete a Lookup by its unique identifier using In-Memory store approach
   * @param actor - The authenticated user context
  ...
  async delete(
    ...
    const correlationId = CorrelationUtil.generateForOperation('lookup-delete');
    ...
    try {
      ...
      if (!lookupStore.has(actor.tenant || 'default-tenant', code.value)) {
        ...
        RepositoryLoggingUtil.logOperationError(
          ...
          operation,
          logContext,
          new Error(`Lookup not found: ${code.value}`),
          'MEDIUM',
        );
      ...
      lookupStore.delete(actor.tenant || 'default-tenant', code.value);

      const receipt: SaveReceipt = {
        stream: `lookup-${code.value}`,
