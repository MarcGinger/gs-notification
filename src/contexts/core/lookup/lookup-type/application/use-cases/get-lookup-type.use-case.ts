...
import { IGetLookupTypeUseCase } from './contracts';
import { LookupTypeErrors } from '../../domain/errors';

/**
 * Get LookupType Use Case - Simplified CQRS Read Side Implementation
 *
 * Simple use case that queries for lookupType data by code.
 * Authorization is handled at the service layer.
 */

@Injectable()
export class GetLookupTypeUseCase implements IGetLookupTypeUseCase {
  ...
  constructor(
    ...
    this.logger = componentLogger(moduleLogger, 'GetLookupTypeUseCase');
  }
  ...
  async execute(params: {
    ...
    if (!code?.trim()) {
      return err({
        code: 'LOOKUP_TYPE.INVALID_CODE' as const,
        title: 'Invalid code',
    ...
    // Handle Option<DetailLookupTypeResponse>
    const lookupTypeDto = Option.isNone(queryResult.value)
      ? null
