...
import { LookupTypeDeletedEvent } from '../../domain/events';
import { ILookupTypeWriter } from '../../application/ports';
/**
 * LookupType Writer Repository - Interface Segregation Principle Implementation
 *
 ...
 *
 * @domain Core Context - LookupType Writer Repository
 * @layer Infrastructure
 * @pattern Repository Pattern + Interface Segregation Principle
 */
export class LookupTypeWriterRepository
  extends BaseWriterRepository
  implements ILookupTypeWriter
{
  ...
  /**
   * Save a LookupType aggregate (create or update) using EventStoreDB-first approach
  ...
  async save(
    ...
    const stream = this.buildStreamName(
      actor.tenant,
      LookupTypeProjectionKeys.getEventStoreStreamPrefix(),
      lookupType.id.value,
    );
    ...
    if (events.length === 0) {
      ...
      const receipt: SaveReceipt = {
        ...
        streamRevision:
          lookupType.version === 0
            ? BigInt(-1)
            : BigInt(lookupType.version - 1),
    ...
    const prevVersion = lookupType.version - eventsToStore.length;
    ...
    const envelopes = this.toEnvelopes(
      ...
      eventsToStore,
      actor,
      'lookup-type-writer-repository',
      LookupServiceConstants.BOUNDED_CONTEXT,
      {
  ...
  /**
   * Delete a LookupType by its unique identifier using EventStoreDB-first approach
   * @param actor - The authenticated user context
  ...
  async delete(
    ...
    const eventMetadata: EventMetadata = {
      ...
      service: 'lookup-type-writer-repository',
      timestampIso: this.clock.nowIso(),
      eventVersion: '1.0',
    ...
    // Create proper LookupTypeDeletedEvent
    const deletedEvent = LookupTypeDeletedEvent.create(
      {
        code: code.value,
    ...
    try {
      Log.debug(
        this.logger,
        'Appending lookup-type deletion event to EventStoreDB',
        {
          ...logContext,
          stream,
          lookupTypeCode: code.value,
      ...
      RepositoryLoggingUtil.logQueryMetrics(
        ...
        logContext,
        {
          ...
          sampleData: {
            ...
            lookupTypeCode: code.value,
            streamRevision: appendResult?.nextExpectedRevision?.toString(),
            eventType: 'LookupTypeDeletedEvent',
          },
        },
      );
