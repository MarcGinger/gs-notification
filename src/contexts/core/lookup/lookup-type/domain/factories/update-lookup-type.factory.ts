...
export function updateLookupTypeAggregateFromSnapshot(
  ...
  if (updateProps.lookupType !== undefined) {
    const lookupTypeResult = LookupTypeLookupType.from(updateProps.lookupType);
    if (!lookupTypeResult.ok) {
      return err(
        withContext(lookupTypeResult.error, {
          operation: 'update_lookup_type_lookup_type_validation',
          correlationId: metadata.correlationId,
          userId: metadata.actor?.userId,
  ...
  if (updateProps.attributes !== undefined) {
    const attributesResult = LookupTypeAttributes.from(updateProps.attributes);
    if (!attributesResult.ok) {
      return err(
        withContext(attributesResult.error, {
          operation: 'update_lookup_type_attributes_validation',
          correlationId: metadata.correlationId,
          userId: metadata.actor?.userId,
