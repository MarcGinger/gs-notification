...
/**
 * LookupXxx Writer Repository - In-Memory Implementation
 *
 * Handles write operations (create, update, delete) for LookupXxx aggregates using in-memory projector
 ...
 *
 * @domain Core Context - LookupXxx Writer Repository (In-Memory)
...
export class LookupXxxWriterRepository implements ILookupXxxWriter {
  ...
  /**
   * Save a LookupXxx aggregate (create or update) using In-Memory store approach
  ...
  async save(
    ...
    const logContext = this.createLogContext(operation, correlationId, actor, {
      ...
      eventCount: lookupXxx.uncommittedEvents?.length ?? 0,
    ...
    const events = lookupXxx.uncommittedEvents ?? [];
    ...
    if (events.length === 0) {
      ...
      const receipt: SaveReceipt = {
        ...
        tenant: actor.tenant,
        eventCount: 0,
        newVersion: lookupXxx.version,
        streamRevision: BigInt(lookupXxx.version),
    ...
    const prevVersion = lookupXxx.version - events.length;
    ...
    try {
      Log.debug(this.logger, 'Saving lookup-xxx to shared store', {
        ...logContext,
        eventCount: events.length,
        prevVersion,
        queryDetails: {
          scope: 'shared_lookup-xxx_store',
          method: 'lookupXxxStore.set',
        },
      });

      // Check if lookup-xxx already exists for version conflict detection
      const existingProjection = lookupXxxStore.get(
        actor.tenant,
        lookupXxx.id.value,
      );
      ...
      const projection: LookupXxxProjection = {
        // LookupXxxProps fields
        lookupType: lookupXxx.entity.lookupType.value,
        code: lookupXxx.entity.code.value,
        name: lookupXxx.entity.name.value,
        description: lookupXxx.entity.description?.value,
        enabled: lookupXxx.entity.enabled?.value,
        attributes: lookupXxx.entity.attributes?.value,
        // BaseProjection fields
        tenant: actor.tenant,
        version: lookupXxx.version,
        createdAt: lookupXxx.entity.createdAt.value,
        updatedAt: this.clock.now(),
        deletedAt: undefined,
        lastStreamRevision: lookupXxx.version.toString(),
      };

      // Save to in-memory store
      lookupXxxStore.set(actor.tenant, lookupXxx.id.value, projection);
  ...
  /**
   * Delete a LookupXxx by its unique identifier using In-Memory store approach
   * @param actor - The authenticated user context
  ...
  async delete(
    ...
    try {
      // Check if lookup-xxx exists
      if (!lookupXxxStore.has(actor.tenant || 'default-tenant', code.value)) {
      ...
      lookupXxxStore.delete(actor.tenant || 'default-tenant', code.value);
