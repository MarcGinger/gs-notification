...
import { LookupXxxDeletedEvent } from '../../domain/events';
import { ILookupXxxWriter } from '../../application/ports';
/**
 * LookupXxx Writer Repository - Interface Segregation Principle Implementation
 *
 ...
 *
 * @domain Core Context - LookupXxx Writer Repository
 * @layer Infrastructure
 * @pattern Repository Pattern + Interface Segregation Principle
 */
export class LookupXxxWriterRepository
  extends BaseWriterRepository
  implements ILookupXxxWriter
{
  ...
  /**
   * Save a LookupXxx aggregate (create or update) using EventStoreDB-first approach
  ...
  async save(
    ...
    const stream = this.buildStreamName(
      actor.tenant,
      LookupXxxProjectionKeys.getEventStoreStreamPrefix(),
      lookupXxx.id.value,
    );
    ...
    if (events.length === 0) {
      ...
      const receipt: SaveReceipt = {
        ...
        streamRevision:
          lookupXxx.version === 0 ? BigInt(-1) : BigInt(lookupXxx.version - 1),
    ...
    const prevVersion = lookupXxx.version - eventsToStore.length;
  ...
  /**
   * Delete a LookupXxx by its unique identifier using EventStoreDB-first approach
   * @param actor - The authenticated user context
  ...
  async delete(
    ...
    const eventMetadata: EventMetadata = {
      ...
      service: 'lookup-xxx-writer-repository',
      timestampIso: this.clock.nowIso(),
      eventVersion: '1.0',
    ...
    // Create proper LookupXxxDeletedEvent
    const deletedEvent = LookupXxxDeletedEvent.create(
      {
        code: code.value,
    ...
    try {
      Log.debug(
        this.logger,
        'Appending lookup-xxx deletion event to EventStoreDB',
        {
          ...logContext,
          stream,
          lookupXxxCode: code.value,
      ...
      RepositoryLoggingUtil.logQueryMetrics(
        ...
        logContext,
        {
          ...
          sampleData: {
            ...
            lookupXxxCode: code.value,
            streamRevision: appendResult?.nextExpectedRevision?.toString(),
            eventType: 'LookupXxxDeletedEvent',
          },
        },
      );
