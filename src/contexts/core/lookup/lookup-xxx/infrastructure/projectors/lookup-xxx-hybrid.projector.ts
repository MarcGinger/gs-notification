...
// LookupXxx Projector - ESDB to PostgreSQL Projection
...
/**
 * LookupXxx Projector using Hybrid PostgreSQL + Redis Shared Infrastructure
 *
  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  constructor(
    ...
    super(
      'lookup-xxx-projector',
      'lookup-xxx-projection',
      baseLogger,
      checkpointStore,
    );
    ...
    Log.info(
      this.logger,
      'LookupXxxProjector initialized with hybrid PostgreSQL + Redis architecture',
      {
        method: 'constructor',
   ...
   */
  onModuleInit(): void {
    Log.info(this.logger, 'Starting LookupXxx Projector with CatchUpRunner', {
      method: 'onModuleInit',
      subscriptionGroup: this.subscriptionGroup,
    });
  ...
  private async projectEvent(
    ...
    try {
      ...
      if (alreadyProcessed) {
        this.logger.debug(
          'LookupXxx already processed - using version hint optimization for ' +
            params.code +
            ' version ' +
            params.version,
        );
  ...
  private async applyEventToDatabase(
    ...
    switch (event.type) {
      case 'CoreLookupLookupXxxCreated.v1':
        return this.handleLookupXxxCreated(queryRunner, event);
      case 'CoreLookupLookupXxxUpdated.v1':
        return this.handleLookupXxxUpdated(queryRunner, event);
      case 'CoreLookupLookupXxxDeleted.v1':
        return this.handleLookupXxxDeleted(queryRunner, event);
      default:
  ...
  /**
   * Generic handler for all lookupXxx events with operation-specific customization
   */
  private async handleLookupXxxEvent(
    queryRunner: QueryRunner,
    event: ProjectionEvent,
    operation: string,
    customizer?: (params: LookupXxxRowParams) => void,
  ): Promise<void> {
    try {
      const params = this.extractLookupXxxParams(event, operation);
      ...
      if (result.length > 0) {
        const mappedResult = LookupXxxRowMappers.mapToProjectionUpsertResult(
          result[0],
        );
        Log.debug(this.logger, `${operation} event projected successfully`, {
          method: 'handleLookupXxxEvent',
          operation,
          eventType: event.type,
          streamId: event.streamId,
  ...
  private async handleLookupXxxCreated(
    ...
    return this.handleLookupXxxEvent(
      queryRunner,
      event,
      'handleLookupXxxCreated',
    );
  }
