...
import { IGetLookupXxxUseCase } from './contracts';
import { LookupXxxErrors } from '../../domain/errors';

/**
 * Get LookupXxx Use Case - Simplified CQRS Read Side Implementation
 *
 * Simple use case that queries for lookupXxx data by code.
 * Authorization is handled at the service layer.
 */

@Injectable()
export class GetLookupXxxUseCase implements IGetLookupXxxUseCase {
  ...
  constructor(
    ...
    this.logger = componentLogger(moduleLogger, 'GetLookupXxxUseCase');
  }
  ...
  async execute(params: {
    ...
    if (!code?.trim()) {
      return err({
        code: 'LOOKUP_XXX.INVALID_CODE' as const,
        title: 'Invalid code',
    ...
    if (!isOk(actorResult)) {
      return err(
        withContext(actorResult.error, {
          correlationId,
          userId: user.sub,
          operation: 'get_lookup_xxx',
          code,
        }),
      );
    }

    // Query for the lookupXxx data
    ...
    // Handle Option<DetailLookupXxxResponse>
    const lookupXxxDto = Option.isNone(queryResult.value)
      ? null
