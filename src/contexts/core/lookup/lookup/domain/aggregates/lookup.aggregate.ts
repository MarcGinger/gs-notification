...
import {
  LookupCode,
  createLookupCreatedAt,
  createLookupUpdatedAt,
  createdAtNow,
  updatedAtNow,
...
import { LookupCreatedEvent, LookupUpdatedEvent } from '../events';
import { LookupErrors } from '../errors';
import { LookupDomainState } from '../state';

/**
 * Domain Aggregate Root: Lookup
 *
 * Represents the Lookup aggregate root in the core domain.
 * Encapsulates lookup business logic, state management, and domain invariants.
 *
 ...
 *
 * @domain Core Context - Lookup Aggregate
 * @layer Domain Aggregates
 */

/**
 * Lookup Aggregate Root
 *
 * Central aggregate root that coordinates lookup business operations.
 * Manages consistency across the aggregate boundary and publishes domain events.
 */
export class LookupAggregate extends AggregateRootBase {
  private _entity: LookupEntity;
  ...
  /**
   * Factory method to create a new Lookup aggregate
   *
   * @param props - Lookup entity properties (should include createdAt, version)
   * @param clock - Clock instance for time operations
   * @param eventMetadata - Event metadata for domain events
   * @returns Result containing LookupAggregate or DomainError
   */
  public static create(
    ...
    const createdAtResult = props.createdAt
      ? createLookupCreatedAt(props.createdAt.value) // Extract Date from existing VO
      : createdAtNow(clock);
    ...
    const updatedAtResult = props.updatedAt
      ? createLookupUpdatedAt(props.updatedAt.value) // Extract Date from existing VO
      : updatedAtNow(clock);
    ...
    const versionResult =
      props.version !== undefined
        ? LookupVersion.create(props.version.value) // Extract number from existing VO
        : initialVersion();
    ...
    const entityResult = LookupEntity.create(entityProps);
    if (!entityResult.ok) {
      return err(entityResult.error);
    }

    // Create the aggregate
    const aggregate = new LookupAggregate(
      entityResult.value,
      clock,
      eventMetadata,
    );

    // Create typed LookupCreatedEvent with only business data
    const createdEvent = LookupCreatedEvent.create({
  ...
  public static reconstitute(
    ...
    const aggregate = new LookupAggregate(entity, clock, eventMetadata);
  ...
  protected when(event: DomainEvent): void {
    switch (event.type) {
      ...
      case 'CoreLookupLookupUpdated.v1': {
        ...
        const entityResult = LookupEntity.fromSnapshot({
  ...
  protected applySnapshot(
    ...
    const entityResult = LookupEntity.fromSnapshot(snapshot);

    if (!entityResult.ok) {
      return err({
        code: 'LOOKUP.SNAPSHOT_APPLICATION_FAILED',
   ...
   */
  public createSnapshot(): LookupSnapshotProps {
   ...
   */
  public toDomainState(): LookupDomainState {
   ...
   */
  public get entity(): LookupEntity {
    return this._entity;
  }
   ...
   */
  private detectChanges(validatedFields: ValidatedLookupUpdateFields): boolean {
   ...
   */
  public updateBatch(
    validatedFields: ValidatedLookupUpdateFields,
    ...
    if (
      ...
      return err({
        ...LookupErrors.CONCURRENCY_CONFLICT,
        context: {
    ...
    const currentVersionResult = createLookupVersion(before.version);
    ...
    const updatedEvent = LookupUpdatedEvent.create({
  ...
  /**
   * Delete the lookup entity (business operation with event publishing)
   *
   * @returns Result indicating success or failure
   */
   ...
   */
  public sameAs(other: LookupAggregate): boolean {
